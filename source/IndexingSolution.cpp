...
void IndexingSolution::setupStandardVectors() {
  ...
  solutionAngleSpread =
      FileParser::getKey("SOLUTION_ANGLE_SPREAD", 8.0) * M_PI / 180;
  ...
  approximateCosineDelta =
      fabs(cos(fortyFiveAngle) - cos(slightlySmallerAngle));
  ...
  if (unitCell.size() < 6) {
    std::cout << "Please supply target unit cell in keyword UNIT_CELL."
              << std::endl;
    exit(1);
  }
...
void IndexingSolution::calculateSimilarStandardVectorsForImageVectors(
    std::vector<SpotVectorPtr> vectors) {
  for (int i = 0; i < vectors.size(); i++) {
    vectors[i]->addSimilarLengthStandardVectors(lattice->getStandardVectors(),
                                                distanceTolerance);
  }
}
...
bool IndexingSolution::vectorPairLooksLikePair(SpotVectorPtr firstObserved,
...
bool IndexingSolution::vectorMatchesVector(
    SpotVectorPtr firstVector, SpotVectorPtr secondVector,
    std::vector<SpotVectorPtr> *firstMatches,
    std::vector<SpotVectorPtr> *secondMatches) {
  MatchPair matchPairs;
  double realAngle = firstVector->angleWithVector(secondVector);

  for (int i = 0; i < uniqueSymVectorCount();
       i++)  // FIXME: standardVectorCount
  {
    double firstVectorTrust =
        firstVector->trustComparedToStandardVector(uniqueSymVector(i));
    ...
    if (firstVectorTrust > firstTolerance) {
      for (int j = 0; j < standardVectorCount(); j++) {
        double secondVectorTrust =
            secondVector->trustComparedToStandardVector(standardVector(j));
        double secondTolerance = secondVector->getMinDistanceTolerance();

        if (secondVectorTrust > secondTolerance) {
          double expectedAngle =
              uniqueSymVector(i)->angleWithVector(standardVector(j));
          ...
          if (difference < angleTolerance) {
            std::pair<SpotVectorPtr, SpotVectorPtr> aPair =
                std::make_pair(uniqueSymVector(i), standardVector(j));
  ...
  for (MatchPair::iterator it = matchPairs.begin(); it != matchPairs.end();
       it++) {
...
bool match_greater_than_match(std::pair<MatrixPtr, double> a,
                              std::pair<MatrixPtr, double> b) {
...
MatrixPtr IndexingSolution::createSolution() {
  ...
  for (SpotVectorMap::iterator it = spotVectors.begin();
       it != spotVectors.end(); it++) {
  ...
  for (int j = 0; j < matrices.size(); j++) {
    ...
    for (int k = 0; k < matrices.size(); k++) {
      ...
      double angle =
          bMat->similarityToRotationMatrix(aMat, solutionAngleSpread);
...
#ifndef __OPTIMIZE__
    logged << matrices[j]->summary() << "\t" << score << "\t" << count
           << std::endl;
#endif
  ...
  std::sort(scoredSolutions.begin(), scoredSolutions.end(),
            match_greater_than_match);
...
#ifndef __OPTIMIZE__
    logged << "Chosen solution:\t" << chosenMat->summary() << "\tfrom "
           << spotVectors.size() << " vectors" << std::endl;
    sendLog(LogLevelDebug);
...
MatrixPtr IndexingSolution::createSolution(SpotVectorPtr firstObserved,
                                           SpotVectorPtr secondObserved,
                                           SpotVectorPtr firstStandard) {
  if (!firstStandard) {
  ...
  // Rotate reciprocal space so that the first simulated vector lines up with
  // the first observed vector.
  MatrixPtr rotateSpotDiffMatrix =
      rotation_between_vectors(simulatedVec1, observedVec1);

  // Rotate this vector by first matrix so that we have treated the two
  // simulated vectors equally
  rotateSpotDiffMatrix->multiplyVector(&simulatedVec2);  // checked

  double resultantAngle = 0;

  // Now we twirl around the firstAxisUnit until the rotated simulated vector
  // matches the second observed vector as closely as possible.

  MatrixPtr secondTwizzleMatrix = closest_rotmat_analytical(
      simulatedVec2, observedVec2, observedVec1, &resultantAngle);

  // We want to apply the first matrix and then the second matrix, so we
  // multiply these.
  rotateSpotDiffMatrix->multiply(*secondTwizzleMatrix);

  // Create the goods.
  MatrixPtr fullMat = MatrixPtr(new Matrix());
  fullMat->setComplexMatrix(lattice->getUnitCellOnly()->copy(),
...
bool IndexingSolution::spotVectorHasAnAppropriateDistance(
    SpotVectorPtr observedVector) {
...
void IndexingSolution::pruneSpotVectors(
...
void IndexingSolution::removeSpotVectors(
    std::vector<SpotVectorPtr>
        *spotVectors) { /*
                           int count = 0;
                           ...
                           {
                               bool exists =
                           this->spotVectors.count((*spotVectors)[i]);
                           ...
                           logged << "Removed " << count << " spot vectors which
                           led to a bad solution." << std::endl; sendLog();*/
}
...
bool IndexingSolution::spotsAreNotTooClose(SpotVectorPtr observedVector) {
  ...
  for (SpotVectorMap::iterator it = spotVectors.begin();
    ...
    if (spot3->closeToSecondSpot(spot1, squareMinDistance) ||
        spot3->closeToSecondSpot(spot2, squareMinDistance) ||
        spot4->closeToSecondSpot(spot1, squareMinDistance) ||
        spot4->closeToSecondSpot(spot2, squareMinDistance))
      return false;
  }
...
bool IndexingSolution::vectorAgreesWithExistingVectors(
    SpotVectorPtr observedVector, SpotVectorPtr standardVector) {
  for (SpotVectorMap::iterator it = spotVectors.begin();
       it != spotVectors.end(); it++) {
    bool similar = vectorPairLooksLikePair(it->first, observedVector,
                                           it->second, standardVector);
...
bool IndexingSolution::vectorSolutionsAreCompatible(
    SpotVectorPtr observedVector, SpotVectorPtr standardVector) {
  int count = 0;
  ...
  for (SpotVectorMap::iterator it = spotVectors.begin();
    ...
    MatrixPtr newSolution =
        createSolution(observedVector, myVector, standardVector);

    for (SpotVectorMatrixMap2D::iterator jt = matrices.begin();
         jt != matrices.end() && count < 5; jt++) {
      for (SpotVectorMatrixMap::iterator kt = jt->second.begin();
           kt != jt->second.end(); kt++) {
...
void IndexingSolution::addMatrix(SpotVectorPtr observedVector1,
                                 SpotVectorPtr observedVector2,
                                 MatrixPtr solution) {
...
void IndexingSolution::addVectorToList(SpotVectorPtr observedVector,
                                       SpotVectorPtr standardVector) {
  spotVectors[observedVector] = standardVector;

  if (spotVectors.size() == 1) return;

  for (SpotVectorMap::iterator i = spotVectors.begin(); i != spotVectors.end();
       i++) {
    if (i->first == observedVector) continue;
...
int IndexingSolution::extendFromSpotVectors(
    std::vector<SpotVectorPtr> *possibleVectors, int limit) {
  int added = 0;

  for (int i = lastUsed; i < possibleVectors->size() && i < lastUsed + 15000;
       i++) {
    ...
    for (SpotVectorMap::iterator it = spotVectors.begin();
      ...
      if (checkingCommonSpots &&
          myVector->hasCommonSpotWithVector(possibleVector)) {
        commonSpots = true;
      }
    ...
    if (checkingCommonSpots && !commonSpots) continue;

    if (duplicates) continue;

    // Find a standard vector which works with all the other vectors
    std::vector<SpotVectorPtr> standardSelection =
        possibleVector->standardVectorsOfSameDistance();

    for (int j = 0; j < standardSelection.size(); j++) {
      bool agrees =
          vectorAgreesWithExistingVectors(possibleVector, standardSelection[j]);

      if (!agrees) continue;

      agrees =
          vectorSolutionsAreCompatible(possibleVector, standardSelection[j]);
      ...
      if (agrees) {
        ...
        if (added == limit && limit > 0) return added;
      }
    }
  }
...
IndexingSolution::IndexingSolution(SpotVectorMap firstMap,
                                   SpotVectorMap secondMap,
                                   SpotVectorMatrixMap2D matrixMap1,
                                   SpotVectorMatrixMap2D matrixMap2,
                                   MatrixPtr symOperator) {
  spotVectors = firstMap;

  for (SpotVectorMap::iterator it = secondMap.begin(); it != secondMap.end();
       it++) {
    SpotVectorPtr myVector = it->first;
...
IndexingSolution::IndexingSolution(SpotVectorPtr firstVector,
                                   SpotVectorPtr secondVector,
                                   SpotVectorPtr firstMatch,
                                   SpotVectorPtr secondMatch) {
  averageTheta = 0;
  averagePhi = 0;
  averagePsi = 0;
  matrixCount = 0;
...
std::vector<IndexingSolutionPtr> IndexingSolution::startingSolutionsForVectors(
    SpotVectorPtr firstVector, SpotVectorPtr secondVector) {
  if (notSetup) {
  ...
  if (!firstMatches.size()) {
    ...
    std::ostringstream logged;
    logged << "Seed matches " << firstMatches.size() << " combinations."
           << std::endl;
    Logger::mainLogger->addStream(&logged, LogLevelDetailed);
  }

  for (int i = 0; i < firstMatches.size(); i++) {
    IndexingSolutionPtr newSolution = IndexingSolutionPtr(new IndexingSolution(
        firstVector, secondVector, firstMatches[i], secondMatches[i]));
...
std::vector<double> IndexingSolution::totalDistanceTrusts() {
  ...
  for (SpotVectorMap::iterator it = spotVectors.begin();
    ...
    if (trust > distanceTolerance * 10) trust = distanceTolerance * 10;
