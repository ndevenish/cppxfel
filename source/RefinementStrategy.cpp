...
RefinementStrategyPtr RefinementStrategy::userChosenStrategy() {
  ...
  switch (method) {
    case MinimizationMethodStepSearch:
      strategy = boost::static_pointer_cast<RefinementStrategy>(
          RefinementStepSearchPtr(new RefinementStepSearch()));
      break;
    case MinimizationMethodNelderMead:
      strategy = boost::static_pointer_cast<RefinementStrategy>(
          NelderMeadPtr(new NelderMead()));
      break;
...
void RefinementStrategy::addParameter(void *object, Getter getter,
                                      Setter setter, double stepSize,
                                      double stepConvergence, std::string tag) {
...
void RefinementStrategy::addCoupledParameter(void *object, Getter getter,
                                             Setter setter, double stepSize,
                                             double stepConvergence,
                                             std::string tag) {
...
void RefinementStrategy::refine() {
  if (!jobName.length()) {
    jobName = "Refinement procedure for " + i_to_str((int)objects.size()) +
              " objects";
  }
...
void RefinementStrategy::reportProgress(double score) {
  if (!(priority <= LogLevelNormal)) return;
...
void RefinementStrategy::finish() {
  ...
  if (endScore >= startingScore || endScore != endScore) {
    logged << "No change for " << jobName << " (" << startingScore << ")"
           << std::endl;
    ...
    if (reduction == reduction) {
      logged << "by " << std::fixed << std::setprecision(4) << reduction * 100
             << "% ";
    }
