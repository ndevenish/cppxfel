...
void NelderMead::addPoints(std::vector<double> *point,
...
void NelderMead::subtractPoints(std::vector<double> *point,
                                std::vector<double> pointToSubtract) {
  for (int i = 0; i < point->size(); i++) {
...
TestPoint NelderMead::reflectOrExpand(std::vector<double> centroid,
                                      double scale) {
  ...
  for (int i = 0; i < testPoints.size(); i++) {
    ...
    for (int j = 0; j < tags.size(); j++) {
      ...
      if (i > 0) {
        ...
        testPoints[i].first[j] =
            testPoints[0].first[j] + (j == minJ) * scale * stepSizes[j];
      }
    }
  }
