...
void UnitCellLattice::getMaxMillerIndicesForResolution(double resolution,
                                                       int *hMax, int *kMax,
                                                       int *lMax) {
  double lengths[3];
  ...
  for (int dist1 = start; dist1 < intervals && dist1 < end; dist1++) {
    ...
    for (int dist2 = 0; dist2 < intervals; dist2++) {
      ...
      for (int angles = 0; angles < intervals; angles++) {
        ...
        int position =
            dist1 * intervals * intervals + dist2 * intervals + angles;
        bestDist = sqrt(bestDist);
        double score = 0;
        double minAngleFunnelRad =
            atan(indexingRlp / chosenDist1) + atan(indexingRlp / chosenDist2);
...
void UnitCellLattice::weightUnitCell() {
  ...
  for (int i = 0; i < uniqueSymVectorCount(); i++) {
    ...
    if (distance > maxAngleDistance) continue;
    ...
    for (int j = 0; j < standardVectorCount(); j++) {
      ...
      if (jDistance > maxAngleDistance) continue;
...
void UnitCellLattice::updateUnitCellData() {
  ...
  std::sort(orderedDistances.begin(), orderedDistances.end(),
            std::less<double>());
}
...
void UnitCellLattice::setup() {
  ...
  if (unitCell.size() < 6) {
    logged
        << "Sorry, need unit cell dimensions specified by UNIT_CELL to do this."
        << std::endl;
    sendLogAndExit();
  }
  ...
  if (!symOperators.size()) {
    Matrix::symmetryOperatorsForSpaceGroup(&symOperators, getSpaceGroup(),
                                           unitCell);
  }
  ...
  getMaxMillerIndicesForResolution(resolution, &maxMillerIndexTrialH,
                                   &maxMillerIndexTrialK,
                                   &maxMillerIndexTrialL);
  ...
  for (int i = -maxMillerIndexTrialH; i <= maxMillerIndexTrialH; i++) {
    for (int j = -maxMillerIndexTrialK; j <= maxMillerIndexTrialK; j++) {
      for (int k = -maxMillerIndexTrialL; k <= maxMillerIndexTrialL; k++) {
        ...
        if (distance > 1 / resolution) continue;

        if (distance > maxDistance) maxDistance = distance;

        SpotVectorPtr newStandardVector =
            SpotVectorPtr(new SpotVector(hkl_transformed, hkl));
...
double UnitCellLattice::weightForPair(double dist1, double dist2,
                                      double angle) {
  if (!weightedAngles) {
...
void UnitCellLattice::refineMtzs(std::vector<MtzPtr> newMtzs) {
  ...
  if (getSpaceGroupNum() < 16) {
    strategy->addParameter(this, getUnitCellBeta, setUnitCellBeta, 0.1, 0.0001,
                           "uc_beta");
  }
