...
class FileParser : public LoggableObject {
 protected:
  ...
  ParserFunction splitLine(std::string line, std::string &command,
                           std::string &rest);
 ...
 public:
    ...
    if (hasKey(key)) {
      ...
      try {
        ...
        return value;
               ...
               boost::exception_detail::error_info_injector<boost::bad_get> >
                   &ex) {
  ...
  FileParser(std::string filename,
             std::vector<std::string> someExtras = std::vector<std::string>());
