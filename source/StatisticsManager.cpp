...
void StatisticsManager::loadFiles(char **filenames, int filenum,
                                  int partiality) {
...
double StatisticsManager::cc_pearson(int num1, int num2, int silent, int *hits,
                                     double *multiplicity, double lowResolution,
                                     double highResolution, bool shouldLog,
                                     bool freeOnly) {
  MtzManager *shot1 = &*(mtzs[num1]);
  MtzManager *shot2 = &*(mtzs[num2]);

  return cc_pearson(shot1, shot2, silent, hits, multiplicity, lowResolution,
                    highResolution, shouldLog, freeOnly);
}

void StatisticsManager::convertResolutions(double lowAngstroms,
                                           double highAngstroms,
                                           double *lowReciprocal,
                                           double *highReciprocal) {
...
double StatisticsManager::cc_pearson(MtzManager *shot1, MtzManager *shot2,
  ...
  if (highResolution == 0) invHigh = FLT_MAX;

  if (lowResolution == 0) invLow = 0;

  if (!silent || shouldLog) {
    std::ostringstream logged;

    if (!silent)
      logged << "h k l\tFirst intensity\tSecond intensity\tResolution"
             << std::endl;

    CSVPtr csv = CSVPtr(new CSV(6, "h", "k", "l", "First intensity",
                                "Second intensity", "Resolution"));
    ...
    for (int i = 0; i < num; i++) {
      ...
      if (freeOnly && !reflections2[i]->miller(0)->isFree()) continue;
  ...
  for (int i = 0; i < num; i++) {
    ...
    if (mean1 != mean1 || mean2 != mean2 || weight != weight) continue;
  ...
  for (int i = 0; i < num; i++) {
    ...
    if (amp_x != amp_x || amp_y != amp_y) continue;
...
double StatisticsManager::r_factor(RFactorType rFactor, MtzManager *shot1,
                                   int *hits, double *multiplicity,
                                   double lowResolution, double highResolution,
                                   bool freeOnly) {
  ...
  for (int i = 0; i < shot1->reflectionCount(); i++) {
    ...
    for (int j = 0; j < n; j++) {
      if (!reflection->miller(j)->accepted()) continue;
      ...
      if (weight != weight || mean_i != mean_i) continue;
    ...
    double one_multiple =
        (double)all_millers_used / (double)all_reflections_used;
    mult += one_multiple;
  ...
  if (hits != NULL) *hits = all_reflections_used;
...
double StatisticsManager::r_split(MtzManager *shot1, MtzManager *shot2,
  ...
  for (int i = 0; i < shot1->reflectionCount(); i++) {
    ...
    if (freeOnly && !reflection2->miller(0)->isFree()) continue;
    ...
    if (int1 == 0 || weight == 0 || weight != weight) continue;
  ...
  if (hits != NULL) *hits = count;

  if (multiplicity != NULL) *multiplicity = mult;
...
double StatisticsManager::cc_through_origin(int num1, int num2, int silent,
  ...
  if (highResolution == 0) highRes = FLT_MAX;

  if (lowResolution == 0) lowRes = 0;
  ...
  for (int i = 0; i < num; i++) {
    ...
    if (mean1 != mean1 || mean2 != mean2) continue;
