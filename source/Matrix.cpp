...
double *Matrix::array() { return components; }
...
void Matrix::setIdentity() {
  for (int i = 0; i < 16; i++) components[i] = 0;

  for (int i = 0; i < 16; i += 5) components[i] = 1;
}
...
std::string Matrix::summary() {
  ...
  dummyVecStr << bDummyVec.h << "\t" << bDummyVec.k << "\t" << bDummyVec.l
...
std::string Matrix::description(bool detailed, bool submatrix) {
  ...
  if (!submatrix) description << "matrix ";
...
void Matrix::eulerAngles(double *theta, double *phi, double *psi, bool force) {
  ...
  *psi = atan2((chosenMat->components[6] / cosTheta),
               (chosenMat->components[10] / cosTheta));
  *phi = atan2((chosenMat->components[1] / cosTheta),
               (chosenMat->components[0] / cosTheta));
...
double Matrix::similarityToRotationMatrix(MatrixPtr mat2, double tolerance,
  ...
  if (thetaDiff > tolerance) return -1;

  double psiDiff = fabs(psi2 - psi1);

  if (psiDiff > tolerance) return -1;

  double phiDiff = fabs(phi2 - phi1);

  if (phiDiff > tolerance) return -1;

  double sumSqr =
      pow(theta1 - theta2, 2) + pow(psi1 - psi2, 2) + pow(phi1 - phi2, 2);
...
void Matrix::symmetryOperatorsForSpaceGroup(std::vector<MatrixPtr> *matrices,
...
void Matrix::unitCellLengths(double *lengths) {
  ...
  double aLengthSqr = pow(mat->components[0], 2) + pow(mat->components[1], 2) +
                      pow(mat->components[2], 2);
  double bLengthSqr = pow(mat->components[4], 2) + pow(mat->components[5], 2) +
                      pow(mat->components[6], 2);
  double cLengthSqr = pow(mat->components[8], 2) + pow(mat->components[9], 2) +
                      pow(mat->components[10], 2);
...
bool Matrix::isIdentity() {
  for (int i = 0; i < 16; i++) {
    if (i == 0 || i == 5 || i == 10 || i == 15) {
      if (fabs(components[i] - 1) > 0.00001) return false;
...
void Matrix::multiply(Matrix &b) { (*this) *= b; }
...
void Matrix::scale(double a) { scale(a, a, a); }
...
bool Matrix::writeToHdf5(std::string address) {
  ...
  bool success =
      processingManager->createDataset(address, 1, &dim, H5T_NATIVE_DOUBLE);

  if (!success) {
    return success;
  }

  return processingManager->writeDataset(address, (void **)&components,
                                         H5T_NATIVE_DOUBLE);
}
...
MatrixPtr Matrix::randomOrientationMatrix() {
  ...
  MatrixPtr unitCell =
      UnitCellLattice::getMainLattice()->getUnitCellOnly()->copy();
