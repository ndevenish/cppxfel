...
class SolventMask {
 private:
  ...
  bool thisSolventMasks(ObjectPtr objectPtr) {
    ...
    return (objectResolution > lowResCutoff &&
            objectResolution < highResCutoff);
  }
 ...
 public:
  ...
  static bool isMasked(ObjectPtr objectPtr) {
    for (int i = 0; i < solventMaskCount(); i++) {
      if (solventMasks[i]->thisSolventMasks(objectPtr)) return true;
    }
  ...
  static int solventMaskCount() { return (int)solventMasks.size(); }
};
