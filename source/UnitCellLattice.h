...
class UnitCellLattice : public FreeLattice,
 ...
 private:
  ...
  UnitCellLattice() : FreeLattice() { setup(); }

 public:
  void setup();
  void getMaxMillerIndicesForResolution(double resolution, int *hMax, int *kMax,
                                        int *lMax);
  void weightUnitCell();
  ...
  CSVPtr getAngleCSV() { return weightedAngles; }

  int standardVectorCount() { return (int)spotVectors.size(); }

  int uniqueSymVectorCount() { return (int)uniqueSymVectors.size(); }

  SpotVectorPtr uniqueSymVector(int i) { return uniqueSymVectors[i]; }

  SpotVectorPtr standardVector(int i) { return spotVectors[i]; }

  int symOperatorCount() { return (int)symOperators.size(); }

  MatrixPtr symOperator(int i) { return symOperators[i]; }

  double getMaxDistance() { return maxDistance; }

  MatrixPtr getUnitCellOnly() { return unitCellOnly; }

  double getMinDistance() { return minDistance; }

  double weightForPair(double dist1, double dist2, double angle);

  std::vector<SpotVectorPtr> getStandardVectors() { return spotVectors; }

  void refineMtzs(std::vector<MtzPtr> newMtzs);

  CSVPtr getWeightedUnitCell() { return weightedUnitCell; }
