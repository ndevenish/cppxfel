...
class MtzMerger : public LoggableObject {
 private:
  ...
  void splitAllMtzs(std::vector<MtzPtr> &firstHalfMtzs,
  ...
  static void writeAnomalousMtz(MtzPtr negative, MtzPtr positive, MtzPtr mean,
                                std::string filename);
 ...
 public:
  ...
  void setScalingType(ScalingType type) { scalingType = type; }

  void setAllMtzs(std::vector<MtzPtr> mtzs) { allMtzs = mtzs; }

  void setSomeMtzs(std::vector<MtzPtr> mtzs) { someMtzs = mtzs; }

  void setExcludeWorst(bool worst) { excludeWorst = worst; }

  void setPreventRejections(bool prevent) { preventRejections = prevent; }

  void setFilename(std::string newName) { filename = newName; }

  std::string getFilename() { return filename; }

  MtzPtr getMergedMtz() { return mergedMtz; }

  void setSilent(bool newSilent) { silent = newSilent; }

  void setFriedel(int newFriedel) { friedel = newFriedel; }

  void setFreeOnly(bool free) { freeOnly = free; }

  void setNeedToScale(bool need) { needToScale = need; }
};
