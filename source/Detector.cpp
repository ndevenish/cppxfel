...
//  Copyright (c) 2016 Division of Structural Biology Oxford. All rights
//  reserved.
//
...
Detector::Detector() { initialiseZeros(); }
...
Detector::Detector(DetectorPtr parent, Coord arrangedTopLeft,
                   Coord arrangedBottomRight, double angle, double offsetX,
                   double offsetY, double gain) {
  initialiseZeros();
  setParent(parent);
  ...
  arrangedMidPoint =
      new_vector((arrangedTopLeft.first + arrangedBottomRight.first) / 2,
  ...
  vec bottomRight =
      new_vector(arrangedBottomRight.first, arrangedBottomRight.second, 0);
  ...
  setUnarrangedTopLeft(std::min(topLeft.h, bottomRight.h),
                       std::min(topLeft.k, bottomRight.k));
  setUnarrangedBottomRight(std::max(topLeft.h, bottomRight.h),
                           std::max(topLeft.k, bottomRight.k));
}
...
void Detector::initialise(Coord unarrangedTopLeft, Coord unarrangedBottomRight,
                          vec slowDir, vec fastDir, vec _arrangedTopLeft,
                          bool lastIsMiddle, bool ghost) {
  setUnarrangedTopLeft(unarrangedTopLeft.first, unarrangedTopLeft.second);
  setUnarrangedBottomRight(unarrangedBottomRight.first,
                           unarrangedBottomRight.second);
  ...
  if (lastIsMiddle) {
    ...
    setArrangedTopLeft(_arrangedTopLeft.h, _arrangedTopLeft.k,
                       _arrangedTopLeft.l);
  }
}
...
void Detector::updateUnarrangedMidPoint() {
  unarrangedMidPointX =
      (double)(unarrangedBottomRightX + unarrangedTopLeftX) / 2;
  unarrangedMidPointY =
      (double)(unarrangedBottomRightY + unarrangedTopLeftY) / 2;
}
...
void Detector::setArrangedTopLeft(double newX, double newY, double newZ) {
  ...
  /* Now we create the arranged top left position (absolute) from CrystFEL
   * (say). */
  vec currentArrangedTopLeft = new_vector(newX, newY, newZ);

  /* Now we add the movement required to establish the middle position
   * (absolute) */
  add_vector_to_vector(&currentArrangedTopLeft,
                       currentArrangedBottomRightOffset);

  /* Now we need to change basis of vectors to know where this is relative to
   * parent */
...
void Detector::recalculateChangeOfBasis(vec *fastAxis, vec *slowAxis) {
  changeOfBasisMat =
      calculateChangeOfBasis(fastAxis, slowAxis, MatrixPtr(new Matrix()));
}

MatrixPtr Detector::calculateChangeOfBasis(vec *fastAxis, vec *slowAxis,
                                           MatrixPtr inv) {
...
void Detector::rotateAxisRecursive(bool fix) {
  ...
  workingBasisMat =
      calculateChangeOfBasis(&fastRotated, &slowRotated, invWorkingBasisMat);
...
void Detector::updateCurrentRotation() { rotateAxisRecursive(); }

// Housekeeping calculations

vec Detector::getRotatedSlowDirection() {
  return new_vector(workingBasisMat->components[3],
                    workingBasisMat->components[4],
                    workingBasisMat->components[5]);
}

vec Detector::getRotatedFastDirection() {
  return new_vector(workingBasisMat->components[0],
                    workingBasisMat->components[1],
                    workingBasisMat->components[2]);
}
...
vec Detector::midPointOffsetFromParent(bool useParent, bool resetNudge) {
  ...
  if (!useParent || resetNudge) {
    ...
    if (resetNudge) {
      if (!isLUCA()) {
        getParent()->getChangeOfBasis()->inverse3DMatrix()->multiplyVector(
            &diff);
...
void Detector::fullDescription() { Detector::getMaster()->description(true); }

void Detector::description(bool propogate) {
  logged << "I am detector " << getTag();
  if (!isLUCA()) logged << " and my parent is " << getParent()->getTag();

  vec midpoint = midPointOffsetFromParent();
  logged << std::endl
         << "My raw midpoint is " << prettyDesc(arrangedMidPoint) << std::endl;
  logged << std::endl
         << "My corrected midpoint is " << prettyDesc(midpoint) << std::endl;
  logged << "Distance of midpoint from origin is " << length_of_vector(midpoint)
         << " z = " << midpoint.l << std::endl;
  logged << "My midpoint relative to my parent is "
         << prettyDesc(midPointOffsetFromParent(false)) << std::endl;
  logged << "My rotation angles are " << std::setprecision(10)
         << prettyDesc(rotationAngles) << std::endl;
  logged << "My rotated fast axis is " << std::setprecision(10)
         << prettyDesc(getRotatedFastDirection()) << std::endl;
  logged << "My rotated slow axis is " << std::setprecision(10)
         << prettyDesc(getRotatedSlowDirection()) << std::endl;

  if (!hasChildren()) {
    vec intersection;
    spotCoordToAbsoluteVec(unarrangedTopLeftX, unarrangedTopLeftY,
                           &intersection);
    logged << "My top left corner maps to " << prettyDesc(intersection)
           << std::endl;
  }

  logged << "I have " << childrenCount() << " children." << std::endl
         << std::endl;
...
double Detector::spotCoordToResolution(double unarrangedX, double unarrangedY,
                                       double wavelength) {
  vec arrangedPos;
...
DetectorPtr Detector::findDetectorPanelForSpotCoord(double xSpot,
                                                    double ySpot) {
  if (hasChildren()) {
    DetectorPtr probe;
...
DetectorPtr Detector::spotToAbsoluteVec(SpotPtr spot, vec *arrangedPos,
  ...
  if (spot->hasDetector()) {
    spot->getDetector()->spotCoordToAbsoluteVec(
        spot->getRawX(), spot->getRawY(), arrangedPos, image);
...
DetectorPtr Detector::findDetectorAndSpotCoordToAbsoluteVec(double xSpot,
                                                            double ySpot,
                                                            vec *arrangedPos) {
  DetectorPtr detector = findDetectorPanelForSpotCoord(xSpot, ySpot);
...
void Detector::intersectionToSpotCoord(vec absoluteVec, double *xSpot,
                                       double *ySpot, ImagePtr image) {
...
void Detector::intersectionWithRay(vec ray, vec *intersection, ImagePtr image) {
  ...
  double t =
      cumulative.h * cross.h + cumulative.k * cross.k + cumulative.l * cross.l;
...
DetectorPtr Detector::intersectionForMiller(MillerPtr miller,
                                            vec *intersection) {
...
DetectorPtr Detector::spotCoordForMiller(MillerPtr miller, double *xSpot,
                                         double *ySpot) {
  ImagePtr image = miller->getImage();
  vec intersection;
...
std::string Detector::writeGeometryFile(int fileCount, int indentCount,
                                        CSVPtr differenceCSV) {
  //  lockNudges();
  std::ostringstream output;

  output << indents(indentCount) << "panel " << getTag() << std::endl;
  output << indents(indentCount) << "{" << std::endl;
  output << indents(indentCount + 1) << "min_fs = " << unarrangedTopLeftX
         << std::endl;
  output << indents(indentCount + 1) << "min_ss = " << unarrangedTopLeftY
         << std::endl;
  output << indents(indentCount + 1) << "max_fs = " << unarrangedBottomRightX
         << std::endl;
  output << indents(indentCount + 1) << "max_ss = " << unarrangedBottomRightY
         << std::endl;
  ...
  output << indents(indentCount + 1) << "midpoint_x = " << arrangedMidPoint.h
         << std::endl;
  output << indents(indentCount + 1) << "midpoint_y = " << arrangedMidPoint.k
         << std::endl;
  output << indents(indentCount + 1) << "midpoint_z = " << arrangedMidPoint.l
         << std::endl;
  output << indents(indentCount + 1)
         << "ghost = " << (hasChildren() ? "true" : "false") << std::endl;
  output << indents(indentCount + 1)
         << "refine = " << (_refinable ? "true" : "false") << std::endl;
  ...
  for (int i = 0; i < childrenCount(); i++) {
    output << std::endl
           << getChild(i)->writeGeometryFile(fileCount, indentCount + 1,
                                             differenceCSV);
  }
...
void Detector::zLimits(double *min, double *max) {
  ...
  std::string geometryList =
      FileParser::getKey("DETECTOR_LIST", std::string("error"));
  csv->writeToFile("image_to_detector_map_" + geometryList + ".csv");
}

// type = 0 = resolution, type = 1 = z limits
void Detector::resolutionOrZLimits(double *min, double *max, double wavelength,
                                   int type, CSVPtr csv) {
  if (isLUCA()) {
    *min = FLT_MAX;
    *max = -FLT_MAX;
  ...
  if (hasChildren()) {
    for (int i = 0; i < childrenCount(); i++) {
      ...
      getChild(i)->resolutionOrZLimits(&myChildMin, &myChildMax, wavelength,
                                       type, csv);
    ...
    resol = spotCoordToResolution(unarrangedTopLeftX, unarrangedTopLeftY,
                                  wavelength);
    updateMinMax(&myMin, &myMax, resol);

    resol = spotCoordToResolution(unarrangedTopLeftX, unarrangedBottomRightY,
                                  wavelength);
    updateMinMax(&myMin, &myMax, resol);

    resol = spotCoordToResolution(unarrangedBottomRightX, unarrangedTopLeftY,
                                  wavelength);
    updateMinMax(&myMin, &myMax, resol);

    resol = spotCoordToResolution(unarrangedBottomRightX,
                                  unarrangedBottomRightY, wavelength);
    updateMinMax(&myMin, &myMax, resol);
    ...
    *max = myMax;
    ...
    csv->addEntry(5, (double)unarrangedTopLeftX, (double)unarrangedTopLeftY,
                  arrangedPos.h, arrangedPos.k, arrangedPos.l);

    spotCoordToAbsoluteVec(unarrangedTopLeftX, unarrangedBottomRightY,
                           &arrangedPos);
    updateMinMax(&lowZ, &highZ, arrangedPos.l);
    csv->addEntry(5, (double)unarrangedTopLeftX, (double)unarrangedBottomRightY,
                  arrangedPos.h, arrangedPos.k, arrangedPos.l);

    spotCoordToAbsoluteVec(unarrangedBottomRightX, unarrangedTopLeftY,
                           &arrangedPos);
    updateMinMax(&lowZ, &highZ, arrangedPos.l);
    csv->addEntry(5, (double)unarrangedBottomRightX, (double)unarrangedTopLeftY,
                  arrangedPos.h, arrangedPos.k, arrangedPos.l);

    spotCoordToAbsoluteVec(unarrangedBottomRightX, unarrangedBottomRightY,
                           &arrangedPos);
    updateMinMax(&lowZ, &highZ, arrangedPos.l);
    csv->addEntry(5, (double)unarrangedBottomRightX,
                  (double)unarrangedBottomRightY, arrangedPos.h, arrangedPos.k,
                  arrangedPos.l);
...
CSVPtr Detector::resolutionHistogram() {
  ...
  if (approxWavelength == 0.) {
    ...
    logged << "Cannot create resolution histogram without an approximate value "
              "for INTEGRATION_WAVELENGTH. Please provide!"
           << std::endl;
    staticLogAndExit(logged);
  }

  std::vector<double> size =
      FileParser::getKey("DETECTOR_SIZE", std::vector<double>(0, 2));
  int xDim = size[0];
  int yDim = size[1];

  CSVPtr csv = CSVPtr(new CSV(0));
  csv->setupHistogram(minResolution, maxResolution, -1000, "Resolution", 1,
                      "numPixels");
  ...
  for (int i = 0; i < yDim; i++) {
    for (int j = 0; j < xDim; j++) {
      vec intersection;
      DetectorPtr aDetector =
          master->findDetectorAndSpotCoordToAbsoluteVec(j, i, &intersection);
...
void Detector::calculateMillerShifts(bool stdev) {
  ...
  for (int i = 0; i < millerCount(); i++) {
    ...
    if (!myMiller || !myMiller->reachesThreshold()) continue;
...
double Detector::millerScore(bool ascii, bool stdev, int number) {
  ...
  requiredRotToOrigin =
      new_vector(bestPix.first * radius, bestPix.second * radius, 0);
  ...
  if (ascii) {
    ...
    if (number < 0) {
      logged << std::endl
             << "ASCII plot of coordinates for " << getTag() << std::endl;
      sendLog();
    ...
    if (number >= 0) {
      ...
      plotMap["filename"] = getTag() + "_miller_" +
                            (stdev ? "pix_" : "recip_") + i_to_str(number);
      plotMap["height"] = "1000";
      plotMap["width"] = "900";
      plotMap["xHeader0"] = "x";
      plotMap["yHeader0"] = "y";
...
void Detector::quickJumpToOrigin() {
  millerScore();
  logged << getTag() << " - " << requiredRotToOrigin.h << ", "
         << requiredRotToOrigin.k << std::endl;
  sendLog();
  resetNudgeBasis();
...
void Detector::getAllSubDetectors(std::vector<DetectorPtr> &array,
                                  bool childrenOnly) {
  for (int i = 0; i < childrenCount(); i++) {
...
void Detector::resetPoke() {
  ...
  vec travelToSecond =
      vector_between_vectors(firstChildMidpoint, secondChildMidpoint);
  ...
  vec horiz =
      new_vector(1, 0, -pokePerpendicularAxis.h / pokePerpendicularAxis.l);
  scale_vector_to_distance(&horiz, 1);
  vec vert = cross_product_for_vectors(pokePerpendicularAxis, horiz);

  double matComponents[9] = {horiz.h, vert.h,  cross.h, horiz.k, vert.k,
                             cross.k, horiz.l, vert.l,  cross.l};
  ...
  pokeLateralAxis =
      cross_product_for_vectors(pokePerpendicularAxis, pokeLongitudinalAxis);
}
...
void Detector::nudgeTiltAndStep(double *nudgeTiltX, double *nudgeTiltY,
                                double *nudgeStep, double *interNudge) {
  double expectedPixels =
      FileParser::getKey("EXPECTED_GEOMETRY_MOVEMENT", 0.02);
  *nudgeStep = expectedPixels;
  ...
  // heading towards zero means we must send the detector BACK, so tilt is
  // opposite sign of step. The step is proportional to both tan(angle). For
  // shorter distances, the nudged step is smaller than for large distances.
...
std::string toCrystFELAxis(vec axis) {
  ...
  aString << (posH ? "+" : "") << axis.h << "x " << (posK ? "+" : "") << axis.k
          << "y " << (posL ? "+" : "") << axis.l << "z" << std::endl;
...
void Detector::addCrystFELInfo(std::ostringstream &geometry, double clen) {
  ...
  vec slow =
      new_vector(workingBasisMat->components[1], workingBasisMat->components[5],
                 workingBasisMat->components[9]);
  vec fast =
      new_vector(workingBasisMat->components[0], workingBasisMat->components[4],
                 workingBasisMat->components[8]);
...
std::string Detector::writeCrystFELFile() {
  ...
  geometry << "; You may insert some things here probably." << std::endl
           << std::endl;
