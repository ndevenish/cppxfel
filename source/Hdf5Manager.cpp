...
int Hdf5Manager::readSizeForTable(Hdf5Table &table, std::string address) {
  // herr_t H5TBread_table( hid_t loc_id, const char *table_name, size_t
  // dst_size,  const size_t *dst_offset, const size_t *dst_sizes, void *dst_buf
  // )
  ...
  // herr_t H5TBget_field_info ( hid_t loc_id, const char *table_name, char
  // *field_names[], size_t *field_sizes, size_t *field_offsets, size_t
  // *type_size  )
  herr_t error =
      H5TBget_table_info(group, table.getTableName(), &nFields, &nRecords);
  ...
  error = H5TBget_field_info(group, table.getTableName(), NULL, NULL, NULL,
                             &recordSize);
  ...
  //    logged << "Allocating " << totalToAllocate << " bytes to read in table."
  //    << std::endl; sendLog();

  return totalToAllocate;
}

bool Hdf5Manager::recordsForTable(Hdf5Table &table, std::string address,
                                  void *data) {
  std::lock_guard<std::mutex> lg(readingHdf5);
  ...
  herr_t error =
      H5TBread_table(group, table.getTableName(), table.getRecordSize(),
                     table.getOffsets(), table.getFieldSizes(), data);
...
bool Hdf5Manager::writeTable(Hdf5Table &table, std::string address) {
  ...
  if (exists) {
    ...
    error = H5TBwrite_records(group, table.getTableName(), 0, maxRecords,
                              table.getRecordSize(), table.getOffsets(),
                              table.getFieldSizes(), table.getData());

    if (error < 0) {
      logged << "Writing error (maxRecords = " << maxRecords << ") " << nRecords
             << ", " << nRecordsNew << std::endl;
      sendLog();
    }
    ...
    if (maxRecords < nRecordsNew) {
      ...
      error = H5TBappend_records(group, table.getTableName(), extraRecords,
                                 table.getRecordSize(), table.getOffsets(),
                                 table.getFieldSizes(), table.getData());
    ...
    error = H5TBdelete_record(group, table.getTableName(),
                              table.getNumberOfRecords(), records_to_delete);
  ...
  //    logged << "Making new table " << table.getTableTitle() << " in group "
  //    << address << std::endl; sendLog();
  ...
  herr_t error =
      H5TBmake_table(title, group, name, nfields, nrecords, recordSize, headers,
                     offsets, types, chunksize, fill_data, compress, data);

  if (error >= 0) {
    //      logged << "Written " << nfields << " fields, " << nrecords << "
    //      records of " << recordSize << " bytes." << std::endl; sendLog();
...
bool Hdf5Manager::createDataset(std::string address, int nDimensions,
                                hsize_t *dims, hid_t type) {
...
bool Hdf5Manager::writeDataset(std::string address, void **buffer, hid_t type) {
  ...
  /* Write the dataset. */
  herr_t status =
      H5Dwrite(dataset_id, type, H5S_ALL, H5S_ALL, H5P_DEFAULT, buffer);

  return (status >= 0);
}

int Hdf5Manager::hdf5MallocBytesForDataset(std::string dataAddress,
                                           void **buffer) {
  size_t sizeSet = 0;
  size_t sizeType = 0;
  ...
  try {
    ...
    if (offset == -1)  // numDims = 2
    {
      ...
      if (numDims == 1) {
        ...
        H5Sselect_hyperslab(space, H5S_SELECT_SET, unsigned_offset, NULL, count,
                            NULL);
...
void Hdf5Manager::groupsWithPrefix(std::vector<std::string> *list,
                                   std::string prefix,
...
void Hdf5Manager::identifiersFromAddress(std::map<std::string, int> *map,
...
int Hdf5Manager::getSubTypeForIndex(std::string address, int objIdx,
                                    H5G_obj_t *type) {
  std::lock_guard<std::mutex> lg(readingHdf5);
...
std::vector<H5G_obj_t> Hdf5Manager::getSubGroupTypes(std::string address) {
  ...
  try {
    ...
    if (groupAll >= 0) H5Gclose(groupAll);
...
std::string Hdf5Manager::concatenatePaths(std::string path1,
                                          std::string path2) {
...
bool Hdf5Manager::createGroupsFromAddress(std::string address) {
  ...
  for (int i = 0; i <= componentCount; i++) {
    ...
    if (exists) {
      ...
      if (success) {
        //      logged << "Address " << pathToCheck << " successfully created."
        //      << std::endl;
      } else {
        //      logged << "Address " << pathToCheck << " creation failed." <<
        //      std::endl;
      }
    }
  }
