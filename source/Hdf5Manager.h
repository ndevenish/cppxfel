...
class Hdf5Manager : public LoggableObject {
 ...
 public:
  ...
  Hdf5Manager(std::string newName,
              Hdf5AccessType hdf5AccessType = Hdf5AccessTypeReadOnly);
  void closeHdf5();
  virtual ~Hdf5Manager();
  ...
  bool createDataset(std::string address, int nDimensions, hsize_t *dims,
                     hid_t type);
  ...
  void identifiersFromAddress(std::map<std::string, int> *map,
  ...
  void groupsWithPrefix(std::vector<std::string> *list, std::string prefix,
