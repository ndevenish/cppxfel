...
Spot::Spot(ImagePtr image) {
  ...
  backgroundPadding =
      FileParser::getKey("IMAGE_SPOT_PROBE_BG_PADDING", probePadding) * 2 + 1;
...
bool Spot::isAcceptable(ImagePtr image) {
  ...
  for (int i = x - tolerance; i < x + tolerance; i++) {
    for (int j = y - tolerance; j < y + tolerance; j++) {
      if (!image->accepted(i, j)) return false;
    }
  }
...
double Spot::focusOnNearbySpot(double maxShift, double trialX, double trialY,
                               int round) {
  ...
  for (int j = -padding; j < padding + 1; j++) {
    for (int i = -padding; i < padding + 1; i++) {
      ...
      double realIntensity =
          this->getParentImage()->valueAt(focusedX + i, focusedY + j);
  ...
  double correlation =
      correlation_between_vectors(&probeIntensities, &realIntensities);
...
void Spot::makeProbe(int height, int background, int length, int backPadding) {
  ...
  if (backPadding < length) backPadding = length;

  if (backPadding % 2 == 0) backPadding++;
  ...
  for (int i = backgroundThickness; i < length + backgroundThickness; i++) {
    for (int j = backgroundThickness; j < length + backgroundThickness; j++) {
      ...
      double distance_from_centre =
          sqrt(from_centre_i * from_centre_i + from_centre_j * from_centre_j);
...
double Spot::angleInPlaneOfDetector(double centreX, double centreY,
                                    vec upBeam) {
...
Coord Spot::getXY() {
  ...
  DetectorPtr detector = Detector::getMaster()->spotToAbsoluteVec(
      shared_from_this(), &arrangedPos);
...
Coord Spot::getRawXY() { return std::make_pair(x, y); }
...
void Spot::recentreInWindow(ImagePtr thisImage, double *x, double *y,
                            int windowPadding) {
  ...
  for (int i = -windowPadding; i < windowPadding + 1; i++) {
    for (int j = -windowPadding; j < windowPadding + 1; j++) {
      ...
      if (thisImage->accepted(myX, myY)) continue;

      int pixelIntensity = thisImage->valueAt(myX, myY);

      if (pixelIntensity <= 10) continue;
...
bool Spot::isOnSameLineAsSpot(SpotPtr otherSpot, double toleranceDegrees) {
  ...
  if (thisAngle < 0) thisAngle += M_PI;

  if (otherAngle < 0) otherAngle += M_PI;

  return (thisAngle > otherAngle - tolerance &&
          thisAngle < otherAngle + tolerance);
}
...
void Spot::writeDatFromSpots(std::string filename, std::vector<SpotPtr> spots) {
  ...
  for (int i = 0; i < spots.size(); i++) {
    ...
    csv.addEntry(1000, estimated.h, estimated.k, estimated.l,
                 spots[i]->angleInPlaneOfDetector(), 1., 1.,
                 spots[i]->getRawXY().first, spots[i]->getRawXY().second, 1.);
...
vec Spot::estimatedVector() {
  ...
  if (hasDetector()) {
    ...
    DetectorPtr det = Detector::getMaster()->spotToAbsoluteVec(
        shared_from_this(), &_estimatedVec);
...
double Spot::integrate() {
  ...
  intensity = getParentImage()->intensityAt(rawXY.first, rawXY.second, shoebox,
                                            &counting);
...
void Spot::spotsAndVectorsToResolution(
  ...
  for (int i = 0; i < spots.size(); i++) {
    if (spots[i]->resolution() < maxResol &&
        spots[i]->resolution() > minResol) {
  ...
  for (int i = 0; i < spotVectors.size(); i++) {
    ...
    if ((firstSpot->resolution() < maxResol &&
         secondSpot->resolution() < maxResol) &&
        (firstSpot->resolution() > minResol &&
         secondSpot->resolution() > minResol)) {
