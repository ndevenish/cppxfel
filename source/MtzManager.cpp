...
void MtzManager::loadParametersMap() {
  optimisingWavelength =
      FileParser::getKey("OPTIMISING_WAVELENGTH", !OPTIMISED_WAVELENGTH);
  optimisingBandwidth =
      FileParser::getKey("OPTIMISING_BANDWIDTH", !OPTIMISED_BANDWIDTH);
  optimisingMosaicity =
      FileParser::getKey("OPTIMISING_MOSAICITY", !OPTIMISED_MOSAICITY);
  optimisingOrientation =
      FileParser::getKey("OPTIMISING_ORIENTATION", !OPTIMISED_ROT);
  optimisingRlpSize =
      FileParser::getKey("OPTIMISING_RLP_SIZE", !OPTIMISED_SPOT_SIZE);
  optimisingExponent =
      FileParser::getKey("OPTIMISING_EXPONENT", !OPTIMISED_EXPONENT);

  resetDefaultParameters();

  stepSizeWavelength = FileParser::getKey("STEP_SIZE_WAVELENGTH", MEAN_STEP);
  stepSizeBandwidth = FileParser::getKey("STEP_SIZE_BANDWIDTH", BANDWIDTH_STEP);
  stepSizeMosaicity = FileParser::getKey("STEP_SIZE_MOSAICITY", MOS_STEP);
  stepSizeRlpSize = FileParser::getKey("STEP_SIZE_RLP_SIZE", SPOT_STEP);
  stepSizeOrientation = FileParser::getKey("STEP_SIZE_ORIENTATION", ROT_STEP);
  stepSizeOrientABC =
      FileParser::getKey("STEP_SIZE_ORIENTATION_ABC", ROT_STEP * 3);
  stepSizeExponent = FileParser::getKey("STEP_SIZE_EXPONENT", EXPONENT_STEP);

  toleranceWavelength =
      FileParser::getKey("TOLERANCE_WAVELENGTH", MEAN_TOLERANCE);
  toleranceBandwidth =
      FileParser::getKey("TOLERANCE_BANDWIDTH", BANDWIDTH_TOLERANCE);
  toleranceMosaicity = FileParser::getKey("TOLERANCE_MOSAICITY", MOS_TOLERANCE);
  toleranceRlpSize =
      FileParser::getKey("TOLERANCE_RLP_SIZE", SPOT_SIZE_TOLERANCE);
  toleranceOrientation =
      FileParser::getKey("TOLERANCE_ORIENTATION", ROT_TOLERANCE);
  toleranceExponent = FileParser::getKey("TOLERANCE_EXPONENT", EXPO_TOLERANCE);
  ...
  int defaultScoreInt =
      FileParser::getKey("DEFAULT_TARGET_FUNCTION", (int)DEFAULT_SCORE_TYPE);
  ...
  maxResolutionAll =
      FileParser::getKey("MAX_REFINED_RESOLUTION", MAX_OPTIMISATION_RESOLUTION);
...
MtzManager::MtzManager() {
  ...
  testBandwidth =
      FileParser::getKey("OVER_PRED_BANDWIDTH", OVER_PRED_BANDWIDTH) / 2;
  testSpotSize = FileParser::getKey("OVER_PRED_RLP_SIZE", OVER_PRED_SPOT_SIZE);
  ...
  initialStep =
      FileParser::getKey("INITIAL_ORIENTATION_STEP", INITIAL_ORIENTATION_STEP);

  searchSize =
      FileParser::getKey("METROLOGY_SEARCH_SIZE", METROLOGY_SEARCH_SIZE);
...
void MtzManager::addMiller(MillerPtr miller) {
  ...
  int reflection_index = Reflection::indexForReflection(
      miller->getH(), miller->getK(), miller->getL(), getSpaceGroup());
...
void MtzManager::getWavelengthFromHDF5() {
  if (!dropped) {
    ...
    std::string noImg = getFilename().substr(
        4, length - 4);  // [img-tag-XXXXX_0 to tag-XXXXX_0]
    unsigned long underscoreIndex = noImg.rfind("_");
    std::string noCryst = noImg.substr(0, underscoreIndex);

    Hdf5ManagerCheetahPtr manager =
        Hdf5ManagerCheetah::hdf5ManagerForImage(noCryst);
    ...
    logged << "Searching for wavelength in " << noCryst
           << " and found wavelength " << aWavelength << std::endl;
    sendLog();
  }
}
...
void MtzManager::loadReflections() {
  ...
  if (getFilename().length() == 0) {
    std::cerr << "Cannot load reflections as no filename has been specified."
              << std::endl;
    return;
  }
  ...
  bool recalculateWavelengths =
      FileParser::getKey("RECALCULATE_WAVELENGTHS", false);
  ...
  float *adata =
      (float *)CCP4::ccp4_utils_malloc((mtz->ncol_read) * sizeof(float));
  ...
  if (col_f == NULL) {
    ...
    if (col_f == NULL) {
      ...
      if (col_f == NULL) col_f = MtzColLookup(mtz, "FC");

      if (col_f != NULL) {
        std::cout << "Warning: using observed amplitude instead of intensity"
                  << std::endl;
      } else {
        std::cout << "Warning: could not find any amplitude columns"
                  << std::endl;
      }
    } else {
      std::cout << "Warning: using calculated amplitude instead of intensity"
                << std::endl;
    }
  }
  ...
  if (col_f == NULL) {
    std::cout
        << "Warning: intensity column not labelled I or IMEAN or amplitude"
        << std::endl;
    exit(1);
  }
  ...
  if (col_sigf == NULL) {
    std::cout << "Warning: sigma column not labelled SIGI or SIGIMEAN"
              << std::endl;
  }
  ...
  if (!fixUnitCell) {
    ...
    vector<double> givenUnitCell =
        FileParser::getKey("UNIT_CELL", vector<double>());
  ...
  for (int i = 0; i < mtz->nref_filein * mtz->ncol_read; i += mtz->ncol_read) {
    ...
    if (col_partials == NULL && intensity != intensity) continue;
    ...
    if (miller->isSpecial()) {
      logged << "Adding chosen Miller from " << getFilename()
  ...
  Logger::mainLogger->addStream(&log,
                                (lowMem ? LogLevelDetailed : LogLevelNormal));

  setSigmaToUnity();

  if (recalculateWavelengths && matrix) this->recalculateWavelengths();
...
void MtzManager::setReference(MtzManager *reference) {
  if (reference != NULL)
    Logger::mainLogger->addString("Setting reference to " +
                                  reference->getFilename());
  MtzManager::referenceManager = reference;
}

ReflectionPtr MtzManager::findReflectionWithId(ReflectionPtr exampleRefl,
                                               size_t *lowestId) {
  ...
  low = std::lower_bound(reflections.begin(), reflections.end(), exampleRefl,
                         Reflection::reflLessThan);
  if (lowestId) *lowestId = (low - reflections.begin());

  if (low < reflections.end() &&
      (*low)->getReflId() == exampleRefl->getReflId()) {
    return *low;
  } else {
    return ReflectionPtr();
  }
...
int MtzManager::findReflectionWithId(long unsigned int refl_id,
  ...
  if ((refl_id < this->reflection(lower)->getReflId()) ||
    ...
    if (insertionPoint) {
      if (refl_id < this->reflection(lower)->getReflId()) return 0;
    ...
    if (new_bound == higher || new_bound == lower) {
      if (insertionPoint) {
        ...
        for (int i = start; i < higher + 2 && i < reflectionCount(); i++) {
          if (this->reflection(i)->getReflId() < refl_id) lowest = i;
        }
...
void MtzManager::findCommonReflections(MtzManager *other,
                                       vector<ReflectionPtr> &reflectionVector1,
                                       vector<ReflectionPtr> &reflectionVector2,
...
MtzPtr MtzManager::getDifferenceManager() {
  ...
  differenceManager->setFilename(
      FileParser::getKey("DIFFERENCE_MTZ", std::string("")));
  differenceManager->loadReflections();

  if (differenceManager->reflectionCount() <= 0) {
    std::ostringstream logged;
    logged << "Difference MTZ required, please specify with DIFFERENCE_MTZ."
           << std::endl;
...
std::vector<double> MtzManager::getDifferencesWith(MtzPtr other,
...
void MtzManager::bFactorAndScale(double *scale, double *bFactor,
                                 double exponent) {
  ...
  this->findCommonReflections(reference, imageReflections, refReflections,
                              NULL);
  ...
  for (int i = 0; i < imageReflections.size(); i++) {
    ...
    if (!imgRef->anyAccepted()) continue;
    ...
    if (logIntensityRatio != logIntensityRatio) continue;

    boost::tuple<double, double, double> point =
        boost::make_tuple(right_exp, logIntensityRatio, imgWeight);
  ...
  logged << "Last scale for " << getFilename() << ": " << k
         << ", bFactor: " << b << std::endl;
...
void MtzManager::scaleToMtz(MtzManager *otherManager, bool info, double lowRes,
                            double highRes) {
  ...
  for (int i = 0; i < num; i++) {
    for (int j = 0; j < reflections1[i]->millerCount(); j++) {
      ...
      if (miller->isFree()) continue;

      if (!miller->accepted()) continue;

      if (!reflections1[i]->betweenResolutions(lowRes, highRes)) continue;

      double int1 = miller->intensity();
      double int2 = reflections2[i]->meanIntensity();
      double weight = miller->getPartiality();

      if ((int1 != int1) || (int2 != int2) || (weight != weight)) continue;
...
void MtzManager::applyScaleFactor(double scaleFactor, double lowRes,
                                  double highRes, bool absolute) {
  ...
  logged << "Applying scale factor to " << getFilename() << " - " << scaleFactor
         << " (now " << this->scale << ")" << std::endl;
...
void MtzManager::writeToFile(std::string newFilename, bool announce,
                             bool plusAmbiguity, bool withScale) {
  int columns = 11;
...
int MtzManager::accepted() {
  ...
  for (int i = 0; i < reflectionCount(); i++) {
    for (int j = 0; j < reflection(i)->millerCount(); j++) {
      if (reflection(i)->miller(j)->accepted()) acceptedCount++;
    }
  }
...
std::string MtzManager::writeParameterSummary() {
  ...
  std::ostringstream summary;
  summary << getFilename() << "," << getRefCorrelation() << ","
          << getLastRSplit() << "," << getRefPartCorrel() << "," << accepted()
          << "," << getTotalReflections() << "," << getMosaicity() << ","
          << getWavelength() << "," << getBandwidth() << "," << hRot << ","
          << kRot << "," << lRot << "," << getSpotSize() << "," << getExponent()
...
void MtzManager::calculateNearbyMillers() {
  ...
  logged << "Integrating to maximum Miller indices: (" << maxMillers[0] << ", "
         << maxMillers[1] << ", " << maxMillers[2] << ") to resolution "
  ...
  if (!spg) {
    logged << "Error: Trying to integrate reflections, but could not load "
              "missing SPACE_GROUP information."
           << std::endl;
    sendLogAndExit();
  }
  ...
  for (int h = -maxMillers[0]; h < maxMillers[0]; h++) {
    for (int k = -maxMillers[1]; k < maxMillers[1]; k++) {
      for (int l = -maxMillers[2]; l < maxMillers[2]; l++) {
        if (ccp4spg_is_sysabs(getSpaceGroup(), h, k, l)) continue;

        vec hkl = new_vector(h, k, l);
        rotatedMatrix->multiplyVector(&hkl);

        if (hkl.l > 0.01) continue;
  ...
  if (minResolutionAll > 0)
    logged << "Rejected " << underRes
           << " due to being under resolution edge of " << minResolutionAll
           << " Å." << std::endl;
...
void MtzManager::checkNearbyMillers() {
  ...
  if (!(unitCell.size() == 6)) {
    logged << "Error: Trying to integrate reflections, but need the UNIT_CELL "
  ...
  if (maxResolutionAll == 0) maxD = FLT_MAX;
  ...
  for (int i = 0; i < nearbyMillers.size(); i++) {
    ...
    miller->crossesBeamRoughly(rotatedMatrix, 0.0, testSpotSize * 2, wavelength,
                               testBandwidth);

    if (i == 0) {
      logged << "Calculated partiality with parameters: hRot " << hRot
             << ", kRot " << kRot << ", spot size " << testSpotSize
  ...
  logged << "Using wavelength " << std::setprecision(9)
         << getImagePtr()->getWavelength() << " Å." << std::endl;
...
bool MtzManager::millerWithinBandwidth(MillerPtr miller) {
  ...
  if (FileParser::hasKey("WAVELENGTH_RANGE")) {
    std::vector<double> range =
        FileParser::getKey("WAVELENGTH_RANGE", std::vector<double>());
...
double MtzManager::hkScore(bool finalise) {
  ...
  if (!finalise) {
    logged << "Score is " << score << " from " << refTotal << " reflections"
           << std::endl;
    sendLog(LogLevelDetailed);
  }
...
void MtzManager::getWavelengthHistogram(vector<double> *wavelengths,
  ...
  double wavelength = getWavelength();
  vector<double> wavelengthRange =
      FileParser::getKey("WAVELENGTH_RANGE", vector<double>(2, 0));
  ...
  for (double i = minLength; i < maxLength; i += interval) {
    ...
    for (int j = 0; j < reflectionCount(); j++) {
      for (int k = 0; k < reflection(j)->millerCount(); k++) {
        ...
        if (ewald < i || ewald > i + interval) continue;
...
void MtzManager::refineOrientationMatrix(bool force) {
  ...
  bool refineOffset =
      FileParser::getKey("REFINE_EACH_DETECTOR_DISTANCE", false);
  ...
  if (refineOrientations || force) {
    RefinementStrategyPtr lStrategy =
        RefinementStepSearchPtr(new RefinementStepSearch());
    lStrategy->setVerbose(Logger::getPriorityLevel() >= LogLevelDetailed);
    lStrategy->setEvaluationFunction(lScoreWrapper, this);
    lStrategy->setJobName("Refining in-detector-plane angle for " +
                          getFilename());
    lStrategy->addParameter(this, getLRot, setLRot, initialStep,
                            orientationTolerance, "lRot");

    if (refineOffset) {
      lStrategy->setJobName(
          "Refining in-detector-plane angle & distance offset for " +
          getFilename());
      lStrategy->addParameter(&*this->getImagePtr(), Image::getDistanceOffset,
                              Image::setDistanceOffset, 1.0, 0.001,
                              "distance_offset");
    }

    if (refineCell) {
      if (refineUnitCellA) {
        lStrategy->addParameter(this, getUnitCellAStatic, setUnitCellAStatic,
                                stepSizeUnitCellA, 0.001, "unit_cell_a");
      }

      if (refineUnitCellB) {
        lStrategy->addParameter(this, getUnitCellBStatic, setUnitCellBStatic,
                                stepSizeUnitCellB, 0.001, "unit_cell_b");
      }

      if (refineUnitCellC) {
        lStrategy->addParameter(this, getUnitCellCStatic, setUnitCellCStatic,
                                stepSizeUnitCellC, 0.001, "unit_cell_c");
      }
    }
    ...
    if (refineOffset) {
      logged << "Detector distance offset for " << getBasename() << " is "
             << Image::getDistanceOffset(&*getImagePtr()) << " pixels."
             << std::endl;
      sendLog();
    }
    ...
    hkStrategy->addParameter(this, getHRot, setHRot, initialStep,
                             orientationTolerance, "hRot");
    hkStrategy->addCoupledParameter(this, getKRot, setKRot, initialStep,
                                    orientationTolerance, "kRot");
    hkStrategy->refine();
  }
...
bool MtzManager::isGoodSolution() {
  bool good = false;
  double goodSolutionSumRatio =
      FileParser::getKey("GOOD_SOLUTION_SUM_RATIO", 6.5);
  int goodSolutionHighestPeak =
      FileParser::getKey("GOOD_SOLUTION_HIGHEST_PEAK", 17);
  int badSolutionHighestPeak =
      FileParser::getKey("BAD_SOLUTION_HIGHEST_PEAK", 10);
  int minimumReflections = FileParser::getKey("MINIMUM_REFLECTION_CUTOFF", 30);
  std::ostringstream details;

  logged << "(" << getFilename() << ") Standard deviation: " << lastStdev
         << std::endl;
  ...
  if (highSum > stdevLow * goodSolutionSumRatio) {
    good = true;
    details << "(" << getFilename() << ") Sum ratio is sufficiently high ("
            << highSum << " vs " << stdevLow << ")" << std::endl;
  }

  if (frequencies.size() && frequencies[0] > goodSolutionHighestPeak) {
    details << "(" << getFilename() << ") Highest peak is high enough ("
            << frequencies[0] << " vs " << goodSolutionHighestPeak << ")"
            << std::endl;
    good = true;
  }

  if (frequencies.size() && frequencies[0] < badSolutionHighestPeak) {
    details << "(" << getFilename() << ") Highest peak is too low ("
            << frequencies[0] << " vs " << badSolutionHighestPeak << ")"
            << std::endl;
    good = false;
  }

  if (getTotalReflections() < minimumReflections) {
    details << "(" << getFilename() << ") However, not enough reflections ("
            << getTotalReflections() << " vs " << minimumReflections << ")"
            << std::endl;
    good = false;
  }
  ...
  for (int i = 0; i < reflectionCount(); i++) {
    for (int j = 0; j < reflection(i)->millerCount(); j++) {
      ...
      if (miller->getPartiality() > partialCutoff) {
        ...
        DetectorPtr myDet;
        myDet =
            Detector::getMaster()->intersectionForMiller(miller, &intersection);
...
void MtzManager::applyScaleFactorsForBins(int binCount) {
  ...
  for (int shell = 0; shell < bins.size() - 1; shell++) {
    ...
    this->findCommonReflections(referenceManager, imgReflections,
                                refReflections, NULL);
    ...
    for (int i = 0; i < imgReflections.size(); i++) {
      if (!imgReflections[i]->anyAccepted()) continue;
