...
RefinementGridSearchPtr GeometryRefiner::makeGridRefiner(
    DetectorPtr detector, GeometryScoreType type) {
  RefinementGridSearchPtr strategy =
      RefinementGridSearchPtr(new RefinementGridSearch());
  ...
  if (type == GeometryScoreTypeInterpanel ||
    ...
    switch (type) {
      case GeometryScoreTypeInterMiller:
        strategy->setEvaluationFunction(Detector::millerScoreWrapper,
                                        &*detector);
        break;
      case GeometryScoreTypeIntraMiller:
        strategy->setEvaluationFunction(Detector::millerStdevScoreWrapper,
                                        &*detector);
        break;
      default:
        break;
...
RefinementStrategyPtr GeometryRefiner::makeRefiner(DetectorPtr detector,
                                                   GeometryScoreType type) {
...
void GeometryRefiner::reportProgress() {
  ...
  IndexManagerPtr intraManager =
      Detector::getMaster()->getIndexManager(GeometryScoreTypeIntrapanel);
  if (!intraManager) {
    intraManager = IndexManagerPtr(new IndexManager(images));
    intraManager->setPseudoScoreType(PseudoScoreTypeIntraPanel);
    Detector::getMaster()->setIndexManager(intraManager,
                                           GeometryScoreTypeIntrapanel);
  }

  IndexManagerPtr interManager =
      Detector::getMaster()->getIndexManager(GeometryScoreTypeInterpanel);
  if (!interManager) {
    interManager = IndexManagerPtr(new IndexManager(images));
    interManager->setPseudoScoreType(PseudoScoreTypeAllInterPanel);
    Detector::getMaster()->setIndexManager(interManager,
  ...
  logged << "N: Progress score (event " << refinementEvent
         << ", intra-panel-dist): " << intraScore << " ("
         << (intraIncrease > 0 ? "+" : "") << intraIncrease
         << "% from last round) " << std::endl;
  logged << "N: Progress score (event " << refinementEvent
         << ", inter-panel-dist): " << interScore << " ("
         << (interIncrease > 0 ? "+" : "") << interIncrease
         << "% from last round)" << std::endl;

  if (mScore != 0) {
    logged << "N: Progress score (event " << refinementEvent
           << ", miller mean): " << mScore << std::endl;
  }
  sendLog();

  manager->powderPattern(
      "geom_refinement_event_" + i_to_str(refinementEvent) + ".csv", false);
...
void GeometryRefiner::refineGeometry() {
  ...
  std::vector<double> sweepDetectorDistance =
      FileParser::getKey("SWEEP_DETECTOR_DISTANCE", std::vector<double>());
  ...
  if (sweepDetectorDistance.size() >= 2) {
    ...
    logged << "***********************************************" << std::endl;
    logged << "Doing a detector distance sweep from " << start << " to " << end
           << " mm." << std::endl;
    logged << "***********************************************" << std::endl;
    sendLog();
    ...
    double newDistance =
        Detector::getArrangedMidPointZ(&*Detector::getMaster());

    logged << "**** Grid search done ****" << std::endl;
    logged << "**** New detector distance: " << newDistance * mmPerPixel
           << " mm. ****" << std::endl;
    sendLog();
  ...
  if (hasMillers && approximate) {
    ...
    for (int i = 0; i < allDetectors.size(); i++) {
      ...
      logged << "Spot count for " << allDetectors[i]->getTag() << " is "
             << spotNum << std::endl;
  ...
  /*
          ...
          logged << "**** Now setting METROLOGY_SEARCH_SIZE to 0 for
     convenience. ****" << std::endl; sendLog();
...
void GeometryRefiner::printHeader(std::vector<DetectorPtr> detectors,
  ...
  if (detectorList.str().length()) {
    logged << "***************************************************"
           << std::endl;
    logged << "  Cycle " << cycleNum << ", event " << refinementEvent
           << std::endl;
    logged << "  Refining detector" << (detectors.size() == 1 ? "" : "s")
           << ": " << detectorList.str() << std::endl;
    logged << "***************************************************" << std::endl
           << std::endl;
    sendLog();
  }
...
void GeometryRefiner::interPanelCycle() {
  ...
  bool solutionApproximate =
      FileParser::getKey("GEOMETRY_IS_APPROXIMATE", false);
...
void GeometryRefiner::addToQueue(DetectorPtr det) {
  ...
  logged << "Adding " << det->getTag() << " back to the queue for cycle "
         << det->getCycleNum() << "." << std::endl;
  logged << "Queue has " << refineQueue.size() << " detectors left."
         << std::endl;
  sendLog();
}
...
void GeometryRefiner::refineDetectorStrategyWrapper(GeometryRefiner *me,
                                                    GeometryScoreType type,
                                                    int strategyType) {
  ...
  for (int i = 0; i < maxThreads; i++) {
    boost::thread *thr =
        new boost::thread(refineDetectorWrapper, me, i, type, strategyType);
...
void GeometryRefiner::refineDetectorWrapper(GeometryRefiner *me, int offset,
                                            GeometryScoreType type,
                                            int strategyType) {
  ...
  while (true) {
    ...
    if (!finished) {
      if ((det->getCycleNum() < maxCycles && maxCycles != 0 &&
        ...
        me->logged << "Finished detector (reached MAXIMUM_CYCLES) for "
                   << det->getTag() << "!" << std::endl;
        me->sendLog();
      ...
      me->logged << "Finished detector (natural ending) " << det->getTag()
                 << "!" << std::endl;
      me->sendLog();
      det->setCycleNum(0);
    }
...
bool GeometryRefiner::refineDetectorStrategy(DetectorPtr detector,
                                             GeometryScoreType type,
                                             int strategyType) {
  std::string typeString = stringForScoreType(type);
...
void GeometryRefiner::peakSearchDetector(DetectorPtr detector) {
  ...
  RefinementGridSearchPtr strategy =
      makeGridRefiner(detector, GeometryScoreTypePeakSearch);
  strategy->setGridLength(gridLength * 2);

  strategy->addParameter(&*detector, Detector::getAddPixelOffsetX,
                         Detector::setAddPixelOffsetX, searchSize / 3, 0.1);
  strategy->addParameter(&*detector, Detector::getAddPixelOffsetY,
                         Detector::setAddPixelOffsetY, searchSize / 3, 0.1);
  strategy->setJobName(detector->getTag() + "_peaksearch");

  strategy->refine();
}

bool GeometryRefiner::interPanelGridSearch(DetectorPtr detector,
                                           GeometryScoreType type) {
  ...
  if (type == GeometryScoreTypeInterMiller) {
    return false;
    strategy->setJobName(detector->getTag() + "_miller_" +
                         i_to_str(rand() % 10000));
    strategy->setEvaluationFunction(Detector::millerScoreWrapper, &*detector);
    strategy->setFinishFunction(NULL);
    strategy->addParameter(&*detector, Detector::getInterNudgeX,
                           Detector::setInterNudgeX,
                           interNudge / nudgeStep * 5.0, 0, "internudge_x");
    strategy->addParameter(&*detector, Detector::getInterNudgeY,
                           Detector::setInterNudgeY,
                           interNudge / nudgeStep * 5.0, 0, "internudge_y");
    ...
    strategy->addParameter(&*detector, Detector::getPokeX, Detector::setPokeX,
                           onePixRot, 0, "poke_x");
    strategy->addParameter(&*detector, Detector::getPokeY, Detector::setPokeY,
                           onePixRot, 0, "poke_y");
...
bool GeometryRefiner::intraPanelMillerSearch(DetectorPtr detector,
                                             GeometryScoreType type) {
  double nudgeStep, nudgeTiltX, nudgeTiltY, interNudge;
  detector->nudgeTiltAndStep(&nudgeTiltX, &nudgeTiltY, &nudgeStep, &interNudge);
...

  {
    ...
    strategy->addParameter(&*detector, Detector::getNudgeZ, Detector::setNudgeZ,
                           nudgeVal, 0, "nudge_z");
  ...
  if (false && solutionApproximate) {
    ...
    // strategy->addParameter(&*detector, Detector::getSmartTiltRatio,
    // Detector::setSmartTiltRatio, 0.005, 0, "tilt_ratio");
    strategy->addParameter(&*detector, Detector::getNudgeTiltX,
                           Detector::setSmartTiltX, nudgeTiltX * 4, 0,
                           "nudgetilt_x");
    strategy->addParameter(&*detector, Detector::getNudgeTiltY,
                           Detector::setSmartTiltY, nudgeTiltY * 4, 0,
                           "nudgetilt_y");
...

  {
    ...
    detector->prepareInterNudges();
    strategy->addParameter(&*detector, Detector::getNudgeTiltX,
                           Detector::setNudgeTiltX, nudgeTiltX / 5, 0,
                           "nudgetilt_x");
    strategy->addParameter(&*detector, Detector::getNudgeTiltY,
                           Detector::setNudgeTiltY, nudgeTiltY / 5, 0,
                           "nudgetilt_y");
...
bool GeometryRefiner::interPanelMillerSearch(DetectorPtr detector,
                                             GeometryScoreType type) {
  double nudgeStep, nudgeTiltX, nudgeTiltY, interNudge;
  detector->nudgeTiltAndStep(&nudgeTiltX, &nudgeTiltY, &nudgeStep, &interNudge);
  ...
  if (type == GeometryScoreTypeInterMiller) {
    ...
    strategy->addParameter(&*detector, Detector::getInterNudgeX,
                           Detector::setInterNudgeX, interNudge * 2, 0,
                           "internudge_x");
    strategy->addParameter(&*detector, Detector::getInterNudgeY,
                           Detector::setInterNudgeY, interNudge * 2, 0,
                           "internudge_y");
    strategy->addParameter(&*detector, Detector::getInterNudgeZ,
                           Detector::setInterNudgeZ, interNudge * 2, 0,
                           "internudge_z");
  } else {
    bool solutionApproximate =
        FileParser::getKey("GEOMETRY_IS_APPROXIMATE", false);
    ...
    strategy->setJobName(detector->getTag() + "_powder");
    strategy->addParameter(&*detector, Detector::getPokeX, Detector::setPokeX,
                           interNudge, 0, "internudge_x");
    strategy->addParameter(&*detector, Detector::getPokeY, Detector::setPokeY,
                           interNudge, 0, "internudge_y");
    strategy->addParameter(&*detector, Detector::getPokeZ, Detector::setPokeZ,
                           interNudge * twizzleMult, 0, "internudge_z");
  }
...
bool GeometryRefiner::refineBeamCentre(DetectorPtr detector) {
  ...
  RefinementGridSearchPtr gridSearch =
      makeGridRefiner(detector, GeometryScoreTypeBeamCentre);
  gridSearch->setJobName("grid_search_beam_centre_" +
                         i_to_str(refinementEvent));
  gridSearch->setGridLength(99);

  double nudgeStep, nudgeTiltX, nudgeTiltY, interNudge;
  detector->nudgeTiltAndStep(&nudgeTiltX, &nudgeTiltY, &nudgeStep, &interNudge);

  gridSearch->addParameter(&*detector, Detector::getInterNudgeX,
                           Detector::setInterNudgeX, interNudge / 5, 0.001,
                           "poke_x");
  gridSearch->addParameter(&*detector, Detector::getInterNudgeY,
                           Detector::setInterNudgeY, interNudge / 5, 0.001,
                           "poke_y");

  if (!detector->isLUCA()) {
    //      gridSearch->addParameter(&*detector, Detector::getInterNudgeZ,
    //      Detector::setInterNudgeY, interNudge, 0.001, "poke_y");
  }
...
void GeometryRefiner::gridSearchDetectorDistance(DetectorPtr detector,
  ...
  RefinementGridSearchPtr strategy =
      RefinementGridSearchPtr(new RefinementGridSearch());
  strategy->addParameter(&*detector, Detector::getArrangedMidPointZ,
                         Detector::setArrangedMidPointZ, step, 0.1, "nudge_z");
...
bool vecToSpotRatioLessThan(ImagePtr one, ImagePtr two) {
  double ratio1 =
      one->spotVectorCount() / (one->spotCount() * one->spotCount());
  double ratio2 =
      two->spotVectorCount() / (one->spotCount() * one->spotCount());
...
void GeometryRefiner::setImages(std::vector<ImagePtr> newImages) {
  ...
  if (cherryPick > 0) {
    ...
    logged << "Cherry picking " << cherryPick << " images from "
           << newImages.size() << std::endl;
    std::sort(newImages.begin(), newImages.end(), vecToSpotRatioLessThan);
    images.clear();
    images.reserve(cherryPick);
    images.insert(images.begin(), newImages.begin(),
                  newImages.begin() + cherryPick);
  } else {
    images = newImages;
  }
