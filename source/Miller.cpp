...
void Miller::setupStaticVariables() {
  ...
  model = FileParser::getKey("BINARY_PARTIALITY", false)
              ? PartialityModelBinary
              : PartialityModelScaled;

  individualWavelength =
      FileParser::getKey("MILLER_INDIVIDUAL_WAVELENGTHS", false);
  ...
  intensityThreshold =
      FileParser::getKey("INTENSITY_THRESHOLD", INTENSITY_THRESHOLD);
...
double Miller::superGaussian(double bandwidth, double mean, double sigma,
                             double exponent) {
  if (mtzParent == NULL || !mtzParent->setupGaussianTable()) {
    return super_gaussian(bandwidth, mean, sigma, exponent);
  } else {
    if (bandwidth != bandwidth || mean != mean) return 0;

    double standardisedX = fabs((bandwidth - mean) / sigma);

    if (standardisedX > MAX_SUPER_GAUSSIAN) return 0;
...
void Miller::recalculatePredictedWavelength() {
  ...
  recalculatePartiality(matrix, mosaicity, rlpSize, wavelength, bandwidth,
                        exponent, false, true);
}
...
Miller::Miller(MtzManager *parent, int _h, int _k, int _l, bool calcFree) {
  ...
  partialCutoff = FileParser::getKey("PARTIALITY_CUTOFF", PARTIAL_CUTOFF);

  int rlpInt = FileParser::getKey("RLP_MODEL", 0);
  rlpModel = (RlpModel)rlpInt;

  std::vector<int> special =
      FileParser::getKey("MILLER_INDEX", std::vector<int>());
...
int Miller::getH() { return hklVector().h; }

int Miller::getK() { return hklVector().k; }

int Miller::getL() { return hklVector().l; }

void Miller::setFlipMatrix(int i) { flipMatrix = i; }

void Miller::setParent(ReflectionPtr reflection) {
  parentReflection = reflection;
}

void Miller::setPartialityModel(PartialityModel _model) { model = _model; }
...
bool Miller::accepted(void) {
  if (this->model == PartialityModelNone) return true;
  ...
  if (this->rawIntensity != this->rawIntensity) return false;
...
bool Miller::isRejected() { return (rejectedReasons > 0); }
...
double Miller::getBFactorScale() {
  ...
  double four_d_squared =
      4 * pow(1 / resn, 2);  // = 1 / (4d^2) = (sinTheta / lambda) ^ 2
...
double Miller::scaleForScaleAndBFactor(double scaleFactor, double bFactor,
  ...
  double four_d_squared =
      4 * pow(resn, 2);  // = 1 / (4d^2) = (sinTheta / lambda) ^ 2
...
double Miller::getPartiality() { return partiality; }

double Miller::getWavelength() { return wavelength; }
...
vec Miller::getTransformedHKL(MatrixPtr myMatrix) {
  if (!myMatrix) myMatrix = getMatrix();
...
void Miller::rotateMatrixHKL(double hRot, double kRot, double lRot,
                             MatrixPtr oldMatrix, MatrixPtr *newMatrix) {
...
void Miller::limitingEwaldWavelengths(vec hkl, double mosaicity,
  ...
  vec move_inwards_position =
      new_vector(inwardsScalar * hkl.h, inwardsScalar * hkl.k,
                 hkl.l - radiusOverLength * newL);
  vec move_outwards_position =
      new_vector(outwardsScalar * hkl.h, outwardsScalar * hkl.k,
                 hkl.l + radiusOverLength * newL);
...
double Miller::calculatePartiality(double pB, double qB, double beamMean,
                                   double beamSigma, double beamExp,
                                   double binary) {
  beamSigma *= beamMean;
  beamSigma /= 2;
...
bool Miller::crossesBeamRoughly(MatrixPtr rotatedMatrix, double mosaicity,
                                double spotSize, double wavelength,
                                double bandwidth) {
  recalculatePartiality(rotatedMatrix, mosaicity, spotSize, wavelength,
                        bandwidth, 1.5, true);
...
void Miller::recalculatePartiality(MatrixPtr rotatedMatrix, double mosaicity,
  ...
  double integral =
      calculatePartiality(pB, qB, wavelength, bandwidth, exponent, binary);
  partiality = integral;

  if (integral > 0 && !binary) {
    double newH = 0;
    double newK =
        sqrt((4 * pow(dStar, 2) - pow(dStar, 4) * pow(wavelength, 2)) / 4);
...
double Miller::getPolarisationCorrection() {
  ...
  if (polarisationCorrection == 0) {
    ...
    double numerator1 =
        1 * verticalFactor + cosSquaredHorizontal * horizontalFactor;
    double denominator1 = verticalFactor + horizontalFactor;

    double numerator2 =
        1 * horizontalFactor + cosSquaredVertical * verticalFactor;
...
void Miller::applyPolarisation(double wavelength) {
  double components[] = {
      -0.0046462837103, 0.00749971430969,  -0.00347981101231,
      0.00531577207698, 0.00576709258158,  0.00533160033255,
      0.00633224815006, 0.000661573996986, -0.00702906145495};
  ...
  if (l == l && l != 0 && std::isfinite(l)) rawIntensity *= l + 1;
}
...
double Miller::getPredictedWavelength() {
  if (!individualWavelength) {
    ...
    if (predictedWavelength != 0) return predictedWavelength;
...
void Miller::positionOnDetector(double *x, double *y, bool shouldSearch) {
  ...
  if (shoebox) even = shoebox->isEven();
  ...
  if (hasDetector()) {
    ...
    detector = Detector::getMaster()->spotCoordForMiller(
        shared_from_this(), &xSpotPred, &ySpotPred);
  }
...
vec Miller::getShiftedRay() {
  if (hasDetector()) {
    ...
    Detector::getMaster()->findDetectorAndSpotCoordToAbsoluteVec(
        correctedX, correctedY, &shiftedRay);
  }

  return shiftedRay;
}

void Miller::makeComplexShoebox(double wavelength, double bandwidth,
                                double mosaicity, double rlpSize) {
...
void Miller::makeShoebox() {
  if (!shoebox) {
    ...
    int neitherLength =
        FileParser::getKey("SHOEBOX_NEITHER_PADDING", SHOEBOX_NEITHER_PADDING);
...
bool Miller::isRejected(RejectReason reason) { return (rejectedReasons > 0); }
...
double Miller::averageRawIntensity(vector<MillerPtr> millers) {
  ...
  for (int i = 0; i < millers.size(); i++) {
    ...
    if (anIntensity != anIntensity) continue;
...
double Miller::getRawestIntensity() { return rawIntensity; }
...
bool Miller::is(int _h, int _k, int _l) {
  long test =
      Reflection::indexForReflection(_h, _k, _l, Reflection::getSpaceGroup());
  long me =
      Reflection::indexForReflection(h, k, l, Reflection::getSpaceGroup());
