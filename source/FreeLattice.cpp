...
void FreeLattice::addExpanded() {
  double reciprocalTolerance =
      FileParser::getKey("RECIPROCAL_TOLERANCE", 0.0015);
...
void FreeLattice::calculateExpandedVectors(bool originOnly) {
  ...
  origin.push_back(
      SpotVectorPtr(new SpotVector(new_vector(0, 0, 0), new_vector(0, 0, 0))));
  ...
  for (int array = 0; array < arrayCount; array++) {
    ...
    for (int i = 0; i < firstVecs->size(); i++) {
      if ((*firstVecs)[i]->distance() > maxDistance) continue;

      for (int j = 0; j < spotVectors.size(); j++) {
        if (spotVectors[j]->distance() > maxDistance) continue;
...
void FreeLattice::anglePattern(bool originOnly) {
  std::vector<double> probeDistances =
      FileParser::getKey("PROBE_DISTANCES", std::vector<double>());
  double distanceTolerance =
      FileParser::getKey("MINIMUM_TRUST_DISTANCE", 1000.);
  double angleStep = FileParser::getKey("POWDER_PATTERN_STEP_ANGLE", 2.0);

  if (probeDistances.size() < 2) return;
  ...
  for (int i = 0; i < expandedSpotVectors.size(); i++) {
    ...
    if (1 / iDiff < distanceTolerance) continue;

    for (int j = 0; j < expandedSpotVectors.size(); j++) {
      double jDiff = fabs(distance2 - expandedSpotVectors[j]->distance());
      if (1 / jDiff < distanceTolerance) continue;

      if (i == j) continue;

      double angle =
          expandedSpotVectors[i]->angleWithVector(expandedSpotVectors[j]) *
          180 / M_PI;
...
FreeLattice::FreeLattice(double a, double b, double c, double alpha,
                         double beta, double gamma) {
  startingAngles(a, b, c, alpha, beta, gamma);
}

// in degrees
void FreeLattice::startingAngles(double a, double b, double c, double alpha,
                                 double beta, double gamma) {
