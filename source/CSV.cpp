...
void CSV::histogram(std::map<double, int> histogram) {
  for (std::map<double, int>::iterator it = histogram.begin();
       it != histogram.end(); it++) {
...
int CSV::findHeader(std::string whichHeader) {
  ...
  if (chosenHeader == -1) {
    logged << "Error: " << whichHeader << " does not exist in table."
           << std::endl;
    sendLogAndExit();
  }
...
double CSV::valueForHistogramEntry(std::string whichHeader, double value,
  ...
  if (categoryNum < 0 || chosenHeader < 0) {
    logged << "Headers asked for in histogram entry (" << whichHeader << ", "
           << categoryHeader << ") are not acceptable." << std::endl;
    sendLog();
  }
...
double CSV::valueForHistogramEntry(int chosenHeader, double value,
                                   int categoryNum) {
  ...
  if (ascending) {
    low = std::lower_bound(histCategories.begin(), histCategories.end(), value,
                           std::less<double>());
  } else {
    low = std::lower_bound(histCategories.begin(), histCategories.end(), value,
                           std::greater<double>());
...
void CSV::addOneToFrequency(double category, int column, double weight,
                            int categoryNum) {
  bool ascending = (entries[0][categoryNum] < entries[1][categoryNum]);

  for (int j = 0; j < entries.size() - 1; j++) {
    if ((ascending && category >= entries[j][categoryNum] &&
         category < entries[j + 1][categoryNum]) ||
        (!ascending && category < entries[j][categoryNum] &&
         category >= entries[j + 1][categoryNum])) {
...
void CSV::addOneToFrequency(double category, std::string whichHeader,
                            double weight, std::string categoryHeader) {
...
void CSV::minMaxCol(int col, double *min, double *max, bool round) {
  ...
  *max = -FLT_MAX;
  ...
  for (int i = 0; i < entries.size(); i++) {
    if (entries[i][col] > *max) *max = entries[i][col];

    if (entries[i][col] < *min) *min = entries[i][col];
  }
...
void CSV::plotPDB(std::string filename, std::string header1,
                  std::string header2, std::string header3) {
  int nums[3];
  ...
  pdbLog << "CRYST1  100.000  100.000  100.000  90.00  90.00  90.00 P 1  "
         << std::endl;
...
void CSV::plotPNG(std::map<std::string, std::string> properties) {
  ...
  while (true) {
    ...
    if (!properties.count("x" + headerKey) ||
        !properties.count("y" + headerKey)) {
      break;
    }
    ...
    if (xCol < 0 || yCol < 0) {
      logged << "Cannot draw graph. Headers incompatible: " << xHeader << ", "
             << yHeader << std::endl;
      sendLog();
      continue;
    }
    ...
    if (properties.count("x" + minKey))
      minX = atof(properties["x" + minKey].c_str());
    if (properties.count("y" + minKey))
      minY = atof(properties["y" + minKey].c_str());
    if (properties.count("x" + maxKey))
      maxX = atof(properties["x" + maxKey].c_str());
    if (properties.count("y" + maxKey))
      maxY = atof(properties["y" + maxKey].c_str());

    if (minX == -FLT_MAX || maxX == FLT_MAX)
      minMaxCol(xCol, &minX, &maxX, false);
    if (minY == -FLT_MAX || maxY == FLT_MAX)
      minMaxCol(yCol, &minY, &maxY, round);
    ...
    for (int i = 0; i < entries.size(); i++) {
      if (entries[i][xCol] != entries[i][xCol] ||
          entries[i][yCol] != entries[i][yCol]) {
        continue;
      }
    ...
    for (int i = 0; i < points.size(); i++) {
      ...
      if (style == GraphStyleScatter) {
        const int length = 1;
        png->drawLine(xProp - length, yProp - length, xProp + length,
                      yProp + length, transparency, red, green, blue);
        png->drawLine(xProp - length, yProp + length, xProp + length,
                      yProp - length, transparency, red, green, blue);
      } else if (style == GraphStyleLine) {
        png->drawLine(lastX, lastY, xProp, yProp, transparency, red, green,
                      blue);
        lastX = xProp;
        lastY = yProp;
      }
...
std::string CSV::plotColumns(int col1, int col2) {
  ...
  writeStringToPlot(headers[col1], &plot, graphLines + bottomMargin - 1,
                    graphCols / 2);
  ...
  for (int i = 0; i < graphCols; i += ((double)graphCols / 10.)) {
    ...
    writeStringToPlot(valueStream.str(), &plot, graphLines + 2,
                      leftMargin + i - 2);
  }
  ...
  for (int i = 0; i < entries.size(); i++) {
    ...
    if (yFullCharsIn == graphLines) yFullCharsIn -= 1;
...
void CSV::addConvolutedPeak(int column, double mean, double stdev,
                            double weight, int categoryNum,
  ...
  for (double x = -stdev * stdevMult / 2; x < stdev * stdevMult / 2;
       x += step) {
    double y = 1;
...
void CSV::convolutedPeaks(std::string category, std::string origHeader,
                          std::string destHeader, double stdev,
                          ConvolutionType type) {
