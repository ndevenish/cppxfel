...
void Hdf5Crystal::writeReflectionData(std::string address) {
  Hdf5ManagerProcessingPtr processingManager =
      Hdf5ManagerProcessing::getProcessingManager();
  ...
  Hdf5Miller *millerData =
      (Hdf5Miller *)malloc(sizeof(Hdf5Miller) * millerCount);
  ...
  logged << "Writing reflection data to HDF5 file was "
         << (success ? "successful." : "a failure.") << std::endl;
...
void Hdf5Crystal::writeCrystalData(std::string address) {
  Hdf5ManagerProcessingPtr manager =
      Hdf5ManagerProcessing::getProcessingManager();

  // space group

  std::string spaceGroupAddress =
      Hdf5Manager::concatenatePaths(address, "spaceGroup");
  manager->writeSingleValueDataset(spaceGroupAddress, getSpaceGroupNum(),
                                   H5T_NATIVE_INT);

  // rotation matrix

  std::string unitCellMatrixAddress =
      Hdf5Manager::concatenatePaths(address, "unitcell_matrix");
  std::string rotationAddress =
      Hdf5Manager::concatenatePaths(address, "rotation_matrix");
  ...
  std::string unitCellParamsAddress =
      Hdf5Manager::concatenatePaths(address, "unitcell_params");

  hsize_t dim = 6;
  manager->createDataset(unitCellParamsAddress, 1, &dim, H5T_NATIVE_DOUBLE);
  manager->writeDataset(unitCellParamsAddress, (void **)&unitCell[0],
                        H5T_NATIVE_DOUBLE);

  // general scaling/indexing ambiguity

  std::string scaleAddress = Hdf5Manager::concatenatePaths(address, "scale");
  std::string ambiguityAddress =
      Hdf5Manager::concatenatePaths(address, "ambiguity");

  manager->writeSingleValueDataset(scaleAddress, scale, H5T_NATIVE_DOUBLE);
  manager->writeSingleValueDataset(ambiguityAddress, activeAmbiguity,
                                   H5T_NATIVE_INT32);
  ...
  std::string mosaicityAddress =
      Hdf5Manager::concatenatePaths(address, "mosaicity");
  std::string rlpSizeAddress =
      Hdf5Manager::concatenatePaths(address, "rlpSize");
  ...
  manager->writeSingleValueDataset(mosaicityAddress, mosaicity,
                                   H5T_NATIVE_DOUBLE);
  ...
  std::string wavelengthAddress =
      Hdf5Manager::concatenatePaths(address, "wavelength");
  std::string bandwidthAddress =
      Hdf5Manager::concatenatePaths(address, "bandwidth");
  std::string exponentAddress =
      Hdf5Manager::concatenatePaths(address, "exponent");

  manager->writeSingleValueDataset(wavelengthAddress, wavelength,
                                   H5T_NATIVE_DOUBLE);
  manager->writeSingleValueDataset(bandwidthAddress, bandwidth,
                                   H5T_NATIVE_DOUBLE);
  manager->writeSingleValueDataset(exponentAddress, exponent,
                                   H5T_NATIVE_DOUBLE);
}
...
void Hdf5Crystal::writeToFile(std::string newFilename, bool announce) {
  ...
  if (manager) {
    ...
    std::string crystalAddress =
        Hdf5Manager::concatenatePaths(address, imgFilename);
...
void Hdf5Crystal::createMillerTable() {
  size_t *fieldOffsets =
      (size_t *)malloc(sizeof(size_t) * HDF5MILLER_FIELD_COUNT);
...
void Hdf5Crystal::loadReflections(PartialityModel model, bool special) {
  ...
  if (address.length() == 0) {
    std::cerr << "Cannot load reflections as no address has been specified."
              << std::endl;
    return;
  }
  ...
  std::string unitCellMatrixAddress =
      Hdf5Manager::concatenatePaths(address, "unitcell_matrix");
  std::string unitCellRotationAddress =
      Hdf5Manager::concatenatePaths(address, "rotation_matrix");
  ...
  success =
      millerTable.readFromManager(manager, reflAddress, (void *)millerData);
  int count = 0;

  bool refined = false;
  std::string refinedAddress =
      Hdf5Manager::concatenatePaths(address, "refined");
  ...
  logged << "Reading reflections from HDF5 file was "
         << (success ? "successful." : "a failure.") << std::endl;
  sendLog();
  ...
  for (int i = 0; i < size; i += sizeof(Hdf5Miller)) {
    ...
    //     std::cout << data->h << ", " << data->k << ", " << data->l << "\t" <<
    //     data->partiality << std::endl;
  ...
  logged << "Loaded " << reflectionCount() << " reflections (" << millerCount()
         << " unique)." << std::endl;
  sendLog();
}
