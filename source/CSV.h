...
class CSV : public LoggableObject {
 ...
 public:
  ...
  void setupHistogram(double start, double end, double interval,
                      std::string catHeader, int count, ...) {
    va_list arguments;
    va_start(arguments, count);
  ...
  void addOneToFrequency(double category, std::string whichHeader,
                         double weight = 1, std::string categoryHeader = "");
  void addOneToFrequency(double category, int column, double weight = 1,
                         int categoryNum = 0);
  ...
  void plotPDB(std::string filename, std::string header1, std::string header2,
               std::string header3);
  ...
  double valueForHistogramEntry(std::string whichHeader, double value,
                                std::string categoryHeader = "");
  double valueForHistogramEntry(int whichHeader, double value,
  ...
  void convolutedPeaks(
      std::string category, std::string origHeader, std::string destHeader,
      double stdev,
      ConvolutionType convolutionType = ConvolutionTypeSuperGaussian);

  void addConvolutedPeak(
      int column, double mean, double stdev, double weight, int categoryNum = 0,
      ConvolutionType convolutionType = ConvolutionTypeSuperGaussian);

  void setValueForEntry(int entry, std::string header, double value);

  void reserveEntries(unsigned long num) { entries.reserve(num); }

  void addEntry(std::vector<double> entry) { entries.push_back(entry); }

  int entryCount() { return (int)entries.size(); }

  std::vector<double> entry(int i) { return entries[i]; }

  double valueForEntry(int headerNum, int entry) {
    return entries[entry][headerNum];
  }

  int headerCount() { return (int)headers.size(); }

  void addHeader(std::string header) { headers.push_back(header); }
