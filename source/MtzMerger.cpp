...
void MtzMerger::splitAllMtzs(std::vector<MtzPtr> &firstHalfMtzs,
                             std::vector<MtzPtr> &secondHalfMtzs) {
  ...
  firstHalfMtzs.insert(firstHalfMtzs.begin(), allMtzs.begin(),
                       allMtzs.begin() + half);
  secondHalfMtzs.insert(secondHalfMtzs.begin(), allMtzs.begin() + half,
                        allMtzs.end());
}
...
void MtzMerger::writeAnomalousMtz(MtzPtr negative, MtzPtr positive, MtzPtr mean,
                                  std::string filename) {
  float cell[6], wavelength, fdata[9];
  int num = 0;
  ...
  for (int i = 0; i < mean->reflectionCount(); i++) {
    ...
    if (posIntensity != posIntensity || negIntensity != negIntensity ||
        meanIntensity != meanIntensity) {
      continue;
    }

    if (meanSigma != meanSigma || posSigma != posSigma ||
        negSigma != negSigma) {
      continue;
    }
...
bool MtzMerger::mtzIsPruned(MtzPtr mtz) {
  ...
  logged << "Rejecting " << mtz->getFilename() << " " << rejectReason
         << std::endl;
...
void MtzMerger::summary() {
  if (!silent) {
    ...
    logged << "N: --------------------------" << std::endl;
    logged << "N: Correlation too low: " << rejectNums[MtzRejectionCorrelation]
           << std::endl;
    logged << "N: R factor too high: " << rejectNums[MtzRejectionRFactor]
           << std::endl;
    logged << "N: Part correl too low: " << rejectNums[MtzRejectionPartCorrel]
           << std::endl;
    logged << "N: Not enough reflections: " << rejectNums[MtzRejectionMinRefl]
           << std::endl;
    logged << "N: Some other reason: " << rejectNums[MtzRejectionOther]
           << std::endl;
    logged << "N: --------------------------" << std::endl;
    logged << "N: Total accepted: " << rejectNums[MtzRejectionNotRejected]
           << std::endl;
    logged << "N: --------------------------" << std::endl;
  ...
  for (int h = -maxMillers[0]; h <= maxMillers[0]; h++) {
    for (int k = -maxMillers[1]; k <= maxMillers[1]; k++) {
      for (int l = -maxMillers[2]; l <= maxMillers[2]; l++) {
        ...
        if (ccp4spg_is_in_asu(spg, h, k, l) &&
            !ccp4spg_is_sysabs(spg, h, k, l)) {
  ...
  for (int j = 0; j < mtz->reflectionCount(); j++) {
    ...
    if (partnerRefl) {
      for (int k = 0; k < refl->millerCount(); k++) {
        ...
        if (accept) {
          if (miller->isSpecial()) {
            logged << "Miller " << prettyDesc(miller->getHKL()) << " from "
                   << miller->getMtzParent()->getFilename() << std::endl;
            logged << "Corrected intensity: " << miller->intensity()
                   << std::endl;
...
void MtzMerger::fixSigmas() {
  ...
  for (int bin = 0; bin < bins.size() - 1; bin++) {
    ...
    for (int i = 0; i < mergedMtz->reflectionCount(); i++) {
      if (!mergedMtz->reflection(i)->betweenResolutions(bins[bin],
                                                        bins[bin + 1])) {
        continue;
      }
    ...
    logged << "<Isigi> for " << bins[bin] << " to " << bins[bin + 1] << " Å is "
           << iOverSigi << std::endl;
    sendLog();
...
void MtzMerger::setCycle(int num) {
  ...
  double corrThreshold =
      FileParser::getKey("CORRELATION_THRESHOLD", CORRELATION_THRESHOLD);
...
MtzMerger::MtzMerger() {
  ...
  partCorrelThreshold =
      FileParser::getKey("PARTIALITY_CORRELATION_THRESHOLD", -2);
  minReflectionCounts = FileParser::getKey("MINIMUM_REFLECTION_CUTOFF",
...
void MtzMerger::merge() {
  if (MtzManager::getReferenceManager()) {
    double refScale =
        1000 / MtzManager::getReferenceManager()->averageIntensity();
  ...
  if (!silent) {
    ...
    logged << "N: Multiplicity: " << (double)observations / (double)totalRefls
           << std::endl;
  ...
  if (!silent) {
    ...
    logged << "N: Completeness: "
           << 100 * (double)someRefls / (double)totalRefls << "%." << std::endl;
  }
...
void MtzMerger::mergeFull(bool anomalous) {
  ...
  if (!idxMerge || !invMerge) {
    logged << "No images in half-data set or both, not doing R split / CC half "
              "calculations."
           << std::endl;
    sendLog();
    doRsplit = false;
  ...
  if (doRsplit) {
    ...
    logged << "N: === R split" << (freeOnly ? " (free)" : "")
           << " ===" << std::endl;
    sendLog();
    double rSplit = idxMerge->rSplitWithManager(&*invMerge, false, false, 0,
                                                maxRes, 20, NULL, true);
    logged << "N: === CC half" << (freeOnly ? " (free)" : "")
           << " ===" << std::endl;
    sendLog();
    double correlation = idxMerge->correlationWithManager(
        &*invMerge, false, false, 0, maxRes, 20, NULL, true);

    logged << "N: Final stats (" << set << "): " << rSplit << ", "
  ...
  logged << "N: Clock time " << minutes << " minutes, " << finalSeconds
         << " seconds to merge (" << set << ")" << std::endl;
  sendLog();
}
...
void MtzMerger::mergeAnomalous() {
  ...
  merge();
  writeAnomalousMtz(negativeMerge, positiveMerge, mergedMtz,
                    makeFilename("anomMerge"));
