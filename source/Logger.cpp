...
bool Logger::isReady() { return mainLogger->ready; }
...
void Logger::addStream(std::ostringstream *stream, LogLevel level,
                       bool shouldExitAfter) {
  if (level > printedLogLevel) return;
  ...
  *ptr << stream->str();
  ...
  if (shouldExitAfter) shouldExit = shouldExitAfter;
  ...
  while (true) {
    ...
    for (StringMap::iterator it = stringsToOutput.begin();
      ...
      for (int i = 0; i < stringsToOutput[it->first].size(); i++) {
        if (stringsToOutput[it->first][i].second > printedLogLevel) continue;
...
void Logger::awaitPrintingWrapper(LoggerPtr logger) { logger->awaitPrinting(); }
