...
void IndexManager::plotGoodVectors() {
  ...
  std::string filename = getActiveDetector()->getTag() + "_" +
                         targetString(getPseudoScoreType()) + "_vec.png";
...
CSVPtr IndexManager::pseudoAnglePDB(bool writePost) {
  ...
  std::string filename = "angles_" + getActiveDetector()->getTag() + "_" +
                         targetString(scoreType) + prePost +
                         i_to_str(getCycleNum()) + ".pdb";
...
PseudoScoreType IndexManager::checkVectors(SpotVectorPtr vec1,
                                           SpotVectorPtr vec2) {
  DetectorPtr activeDetector = getActiveDetector();
  bool isInterPanel = scoreType == PseudoScoreTypeAllInterPanel ||
                      scoreType == PseudoScoreTypeInterPanel;
  ...
  if ((!vec1->originalDistanceLessThan(intraPanelDistance) ||
       !vec2->originalDistanceLessThan(intraPanelDistance))) {
    return PseudoScoreTypeInvalid;
  }
  ...
  if (!isInterPanel && vec1->isIntraPanelVector() &&
      vec2->isIntraPanelVector()) {
    if (vec1->isOnlyFromDetector(activeDetector) &&
        vec2->isOnlyFromDetector(activeDetector)) {
  ...
  if ((vec1->spansChildrenOfDetector(activeDetector) &&
       (vec2->spansChildrenOfDetector(activeDetector)))) {
    return PseudoScoreTypeInterPanel;
  }

  if ((vec1->isOnlyFromDetector(activeDetector) &&
       (vec2->isOnlyFromDetector(activeDetector)) &&
       ((vec1->spansChildrenOfDetector(activeDetector) ||
         (vec1->spansChildrenOfDetector(activeDetector)))) &&
...
double IndexManager::pseudoAngleScore(void *object) {
  ...
  for (std::set<SpotPtr>::iterator it = me->goodSpots.begin();
       it != me->goodSpots.end(); it++) {
    SpotPtr spot = *it;
...
void IndexManager::indexThread(IndexManager *indexer,
                               std::vector<MtzPtr> *mtzSubset, int offset) {
  ...
  while (true) {
    ...
    logged << "Starting image " << image->getFilename() << " on thread "
           << offset << std::endl;
    Logger::mainLogger->addStream(&logged);
    logged.str("");
...
void IndexManager::index() {
  ...
  for (int num = 0; num < 1; num++) {
    ...
    for (int i = 0; i < maxThreads; i++) {
      boost::thread *thr =
          new boost::thread(indexThread, this, &managerSubsets[i], i);
  ...
  for (int i = 0; i < maxThreads; i++) {
    mtzs.insert(mtzs.begin() + lastPos, managerSubsets[i].begin(),
                managerSubsets[i].end());
...
PowderHistogram IndexManager::generatePowderHistogram(int intraPanel,
  ...
  for (int i = 0; i < lattice->standardVectorCount(); i++) {
    ...
    if (categoryNum > maxCategory) continue;

    for (int i = categoryNum - perfectPadding;
         i <= categoryNum + perfectPadding; i++) {
      if (i < 0 || i > maxCategory) continue;
  ...
  for (int i = 0; i < images.size(); i++) {
    for (int j = 0; j < images[i]->spotVectorCount(); j++) {
      ...
      if (categoryNum > maxCategory || categoryNum < 0) continue;
...
void IndexManager::powderPattern(std::string csvName, bool force) {
  ...
  CSV powder(5, "Distance", "Frequency", "Perfect frequency", "Intra-panel",
             "Inter-panel");

  for (PowderHistogram::iterator it = allFrequencies.begin();
       it != allFrequencies.end(); it++) {
