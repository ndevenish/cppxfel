...
void finishJobNotification(int argc, char *argv[], int minutes) {
  ...
  notification << "osascript -e 'display notification \"" << workingDirectory
               << "\" with title \"" << command.str() << "\" subtitle \""
               << minutes << " minutes to complete\" sound name \"Glass\"'"
               << std::endl;

  std::ofstream jobNotificationFile;
  jobNotificationFile.open(jobNotificationFileStr,
...
void new_main(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    ...
    std::cout << "Please refer to & cite paper (Ginn et al., J. Appl. Cryst. "
                 "(2016). 49, 1065-1072)"
              << std::endl
              << std::endl;
    ...
    ;
    std::cout << "Other functions for assessing data quality:" << std::endl
              << std::endl;

    std::cout << "Correlation between two MTZ files:" << std::endl << std::endl;
    std::cout << "\tcppxfel.run -cc firstFile.mtz secondFile.mtz [ambiguity] "
                 "[lowRes] [highRes] [bins]"
              << std::endl
              << std::endl;
    std::cout << "ambiguity: 0, 1, 2 or 3 - will compare different indexing "
                 "solutions where the Bravais lattice symmetry is higher than "
                 "that of the point group for certain space groups. Default 0"
              << std::endl;
    std::cout << "lowRes and highRes: set to resolution in Angstroms to bound "
                 "the results, or set to 0 to take lowest/highest resolution "
                 "data. Default 0, 0"
              << std::endl;
    std::cout
    ...
    std::cout << "\tcppxfel.run -partiality reference.mtz "
                 "ref-img-shot-number.mtz [highRes]"
              << std::endl
              << std::endl;
    std::cout << "highRes: - highest resolution reflection to report results "
                 "on. Defaults to the edge of the image."
              << std::endl;
    std::cout << "This outputs ref-img-shot-number.mtz_partiality_[r].png "
                 "where r denotes the resolution range, and also the relevant "
                 "CSV with the .csv file extension."
              << std::endl
              << std::endl;
    ;
    ...
    std::cout << "General help for most of the parameters which are specified "
                 "in input files can be found using --help or -h, for example:"
              << std::endl;
    std::cout << "\tcppxfel.run --help                       (full list)"
              << std::endl;
    std::cout << "\tcppxfel.run --help verbosity_level" << std::endl;
    std::cout << "\tcppxfel.run --help intensity_threshold" << std::endl;

    std::cout << "\nNow testing your CCP4 library installation..." << std::endl
              << std::endl;
    ...
    if (!spg) {
      std::cout << std::endl
                << "*****************************************************"
                << std::endl;
      std::cout << "************* WARNING! VERY IMPORTANT!! *************"
                << std::endl;
      std::cout << "*****************************************************"
                << std::endl;
      std::cout << std::endl
                << "I can't seem to load space group P1!" << std::endl;
      std::cout << "If CCP4 is installed, it may need sourcing." << std::endl;
      std::cout << "Please make sure the value of the environment variable "
                   "$SYMINFO is set correctly."
                << std::endl;
      std::cout << "Something like (bash): export "
                   "SYMINFO=/path/to/ccp4-vX.X.X/lib/data/syminfo.lib"
                << std::endl;
      std::cout << "Something like (csh): setenv SYMINFO "
                   "/path/to/ccp4-vX.X.X/lib/data/syminfo.lib"
                << std::endl;
      std::cout
          << std::endl
          << "(... look carefully, those paths are almost certainly wrong!)"
          << std::endl;
    } else {
  ...
  boost::thread thr =
      boost::thread(Logger::awaitPrintingWrapper, Logger::mainLogger);
  ...
  if (strcmp(argv[1], "-cc") == 0 || strcmp(argv[1], "-rsplit") == 0) {
    ...
    if (inverted) mtz1->setActiveAmbiguity(1);

    if (rsplit) {
      mtz1->rSplitWithManager(mtz2, 1, 0, lowRes, highRes, bins);
    } else {
      mtz1->correlationWithManager(mtz2, 1, 0, lowRes, highRes, bins, NULL,
                                   true);
    }
  ...
  if (strcmp(argv[1], "-intensities") == 0) {
    if (argc <= 5) {
      std::cout << "arguments: -intensities h k l <file1> {<file2> ...}."
                << std::endl;
      exit(1);
    }
    ...
    std::cout << "Plotting intensities for (" << h << ", " << k << ", " << l
              << ")" << std::endl;
  ...
  if (strcmp(argv[1], "-partiality") == 0) {
    if (argc <= 3) {
      std::cout << "arguments: -partiality <ref> <filein> {<maxres>}."
                << std::endl;
      exit(1);
    }
  ...
  if (strcmp(argv[1], "-bfactor") == 0) {
    if (argc <= 2) {
      std::cout << "arguments: -bfactor <ref> <file1> {<file2> ...}."
                << std::endl;
      exit(1);
    }
  ...
  std::ostringstream logged;
  logged << "N: Total time: " << minutes << " minutes, " << finalSeconds
         << " seconds (" << seconds << " seconds)." << std::endl;

  if (strcmp(argv[1], "-i") == 0) finishJobNotification(argc, argv, minutes);
