...
int Reflection::ambiguityCount() {
  ...
  if (spgNum >= 195 && spgNum <= 199) basicNum = 2;

  if (spgNum >= 168 && spgNum <= 173) basicNum = 2;

  if (spgNum == 146) basicNum = 2;

  if (spgNum >= 149 && spgNum <= 154) basicNum = 3;

  if (spgNum >= 143 && spgNum <= 145) basicNum = 4;
...
void Reflection::setSpaceGroup(int spaceGroupNum) {
  ...
  std::vector<double> ambiguityDouble =
      FileParser::getKey("CUSTOM_AMBIGUITY", std::vector<double>());
  ...
  for (int i = 0; i < ambiguityCount(); i++) {
    ...
    if (success == 0) {
      std::cout << "Major problem: cannot put " << h << " " << k << " " << l
                << " into the asymmetric unit for space group "
                << ccp4_space_group->symbol_Hall << std::endl;
      exit(0);
    }
...
Reflection::Reflection() {
  ...
  rejectSigma =
      FileParser::getKey("OUTLIER_REJECTION_SIGMA", OUTLIER_REJECTION_SIGMA);
...
MillerPtr Reflection::acceptedMiller(int num) {
  ...
  for (int i = 0; i < millers.size(); i++) {
    if (miller(i)->accepted() && accepted == num) return miller(i);

    if (miller(i)->accepted()) accepted++;
  }

  return MillerPtr();
}

MillerPtr Reflection::miller(int i) { return millers[i]; }
...
bool Reflection::betweenResolutions(double lowAngstroms, double highAngstroms) {
  double minD, maxD = 0;
  StatisticsManager::convertResolutions(lowAngstroms, highAngstroms, &minD,
                                        &maxD);

  if (resolution > maxD || resolution < minD) return false;

  return true;
}

int Reflection::millerCount() { return (int)millers.size(); }
...
double Reflection::meanIntensity(bool withCutoff, int start, int end) {
  if (end == 0) end = withCutoff ? acceptedCount() : millerCount();
...
double Reflection::meanIntensityWithExclusion(std::string *filename, int start,
                                              int end) {
  if (filename == NULL) return meanIntensity(true, start, end);

  if (end == 0) end = acceptedCount();
  double total_intensity = 0;
  double weight = 0;
...
double Reflection::mergeSigma() {
  ...
  for (int i = 0; i < millers.size(); i++) {
    if (!millers[i]->accepted()) continue;
...
double Reflection::signalToNoise() { return meanIntensity() / meanSigma(); }
...
MatrixPtr Reflection::getFlipMatrix(int i) {
  if (flipMatrices.size() == 0) return Matrix::getIdentityPtr();
...
void Reflection::reflectionDescription() {
  ...
  for (int i = 0; i < millerCount(); i++) {
    ...
    logged << miller->getH() << "\t" << miller->getK() << "\t" << miller->getL()
           ...
           << miller->getPartiality() << "\t" << miller->getSigma() << "\t"
           << miller->getMtzParent()->getFilename() << std::endl;
    if (miller->accepted()) acceptedCount++;
  }
  logged << std::endl;
...
Reflection::~Reflection() { clearMillers(); }
...
double Reflection::rMergeContribution(double *numerator, double *denominator) {
  ...
  for (int i = 0; i < millerCount(); i++) {
    ...
    if (intensity < 0 || mean_intensity < 0) continue;
...
void Reflection::medianMerge(double *intensity, double *sigma, int *rejected,
                             signed char friedel) {
  ...
  *sigma = standard_deviation(&intensities);
  ...
  /*
  std::ostringstream logged;
  logged << "Int " << *intensity << " from " << intensities.size() << "
  reflections for (" << miller(0)->getH() << ", " << miller(0)->getK() << ", "
  << miller(0)->getL() << ")" << std::endl; Logger::log(logged);*/
}

void Reflection::liteMerge(double *intensity, double *countingSigma,
                           double *sigma, int *rejected, signed char friedel) {
  std::vector<double> intensities, weights;
  ...
  for (int i = 0; i < liteMillers.size(); i++) {
    ...
    if (isSpecial) {
      std::ostringstream logged;
      logged << " MILLER " << liteMillers[i].intensity << " "
             << liteMillers[i].weight << std::endl;
      Logger::log(logged);
    }
  }
...
void Reflection::merge(WeightType weighting, double *intensity, double *sigma,
  ...
  if (isSpecial) {
    for (int i = 0; i < millerCount(); i++) {
      logged << miller(i)->getH() << " " << miller(i)->getK() << " "
             << miller(i)->getL() << " "
             << miller(i)->getMtzParent()->getFilename() << " "
             << miller(i)->intensity() << " " << miller(i)->getRawestIntensity()
             << " " << miller(i)->getScale() << " "
             << miller(i)->getPartiality() << " " << miller(i)->getWeight()
             << std::endl;
    }
  ...
  if (shouldReject) {
    ...
    logged << "Rejected " << rejectedCount << " reflections between "
  ...
  *sigma = newSigma;
  ...
  Logger::mainLogger->addStream(&logged,
                                isSpecial ? LogLevelNormal : LogLevelDebug);
}
...
bool Reflection::anyAccepted() {
  for (int i = 0; i < millers.size(); i++) {
    if (miller(i)->accepted()) return true;
  }
...
void Reflection::detailedDescription() {
  ...
  for (int i = 0; i < millerCount(); i++) {
    ...
    std::cout << miller(i)->getPartiality() << "\t" << fraction << "\t"
              << rawIntensity << "\t" << isym << std::endl;
  }
...
void Reflection::addLiteMiller(MillerPtr miller) {
  ...
  if (isSpecial) {
    std::ostringstream logged;
    logged << "Special: " << intensity << ", " << weight << ", "
           << miller->getRawestIntensity() << ", " << miller->getPartiality()
           << ", " << miller->getScale() << std::endl;
    Logger::log(logged);
  }
