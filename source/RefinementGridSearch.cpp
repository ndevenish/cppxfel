...
void RefinementGridSearch::recursiveEvaluation(ParamList referenceList,
                                               ParamList workingList,
                                               ResultMap *results) {
...
void RefinementGridSearch::assignInterpanelMinimum() {
  ...
  for (int x = 0; x < gridLength; x++) {
    ...
    for (int y = 0; y + gridJumps < gridLength; y++) {
      ...
      if (lineMin - coverPadding < 0) continue;

      if (lineMin + coverPadding >= gridLength) continue;
