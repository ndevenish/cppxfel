...
void Hdf5Image::failureMessage() {
  logged << "(" << getBasename() << ") Unable to open image from HDF5 file."
         << std::endl;
  sendLog();
}
...
void Hdf5Image::loadCrystals() {
  ...
  std::vector<std::string> addresses =
      processingManager->getSubGroupNames(address);
...
void Hdf5Image::loadImage() {
  ...
  /*
          if (!manager->datasetExists(address) &&
     !manager->groupExists(address))
          {
                  logged << "Cannot find an image with data address " <<
     getBasename() << ", nevermind." << std::endl; sendLog(); return;
          }*/
...
void Hdf5Image::writeSpotsList(std::string spotFile) {
  ...
  logged << "Writing spots to HDF5 file was "
...
void Hdf5Image::processSpotList() {
  ...
  if (!forceSpotFinding && processingManager->datasetExists(concat)) {
    ...
    bool success =
        spotTable.readFromManager(processingManager, concat, (void *)spotData);
    int count = 0;

    logged << "Reading spots from HDF5 file was "
           << (success ? "successful." : "a failure.") << std::endl;
    sendLog();
...
void Hdf5Image::createSpotTable() {
  size_t *fieldOffsets =
      (size_t *)malloc(sizeof(size_t) * HDF5SPOT_FIELD_COUNT);
  ...
  const char *toCopy[HDF5SPOT_FIELD_COUNT] = {"x", "y", "intensity", "sigma"};
...
void Hdf5Image::getWavelengthFromHdf5() {
  ...
  /*
          if (!manager->groupExists(address) &&
     !manager->datasetExists(address))
          {
                  return;
          }
  */
