...
typedef enum { TrustLevelGood, TrustLevelAverage, TrustLevelBad } TrustLevel;

class Miller;

class MtzManager : public LoggableObject,
                   public hasFilename,
                   public hasSymmetry,
                   public boost::enable_shared_from_this<MtzManager> {
 protected:
  /* From IOMRefiner */
 ...
 public:
  ...
  ReflectionPtr findReflectionWithId(ReflectionPtr exampleRefl,
                                     size_t *lowestId = NULL);
  int findReflectionWithId(long unsigned int refl_id, ReflectionPtr *reflection,
  ...
  void findCommonReflections(MtzManager *other,
                             ...
                             bool preserve = false);
  void scaleToMtz(MtzManager *otherManager, bool withCutoff = false,
  ...
  void applyScaleFactor(double scaleFactor, double lowRes = 0,
                        double highRes = 0, bool absolute = false);
  void applyScaleFactorsForBins(int binCount);
  double averageIntensity();
  void setSigmaToUnity();
  void replaceBeamWithSpectrum();

  std::vector<double> getDifferencesWith(MtzPtr other,
  ...
  virtual void writeToFile(std::string newFilename, bool announce = false,
                           bool plusAmbiguity = false, bool withScale = false);
  void writeToHdf5();

  double correlationWithManager(
      MtzManager *otherManager, bool printHits = false, bool silent = true,
      double lowRes = 0, double highRes = 0, int bins = 20,
      vector<boost::tuple<double, double, double, int> > *correlations = NULL,
      bool shouldLog = false, bool freeOnly = false);

  double rSplitWithManager(
      MtzManager *otherManager, bool printHits = false, bool silent = true,
      double lowRes = 0, double highRes = 0, int bins = 20,
      vector<boost::tuple<double, double, double, int> > *correlations = NULL,
      bool shouldLog = false, bool freeOnly = false);

  double statisticsWithManager(
  ...
  double rFactorWithManager(
      RFactorType rFactor, bool printHits = false, bool silent = true,
      double lowRes = 0, double highRes = 0, int bins = 20,
      vector<boost::tuple<double, double, double, int> > *correlations = NULL,
  ...
  double correlation(bool silent = true, double lowResolution = 0,
                     double highResolution = -1);
  ...
  float getLastRSplit() { return lastRSplit; }

  static bool setupGaussianTable() { return setupSuperGaussian; }

  int getActiveAmbiguity() { return activeAmbiguity; }

  void setActiveAmbiguity(int newAmbiguity);

  virtual int reflectionCount() { return (int)reflections.size(); }

  virtual ReflectionPtr reflection(int i) { return reflections[i]; }
  ...
  double getMosaicity() const { return mosaicity; }

  void setMosaicity(double mosaicity) { this->mosaicity = mosaicity; }

  double getSpotSize() const { return spotSize; }

  void setSpotSize(double spotSize) { this->spotSize = spotSize; }
  ...
  double getRefCorrelation() const { return refCorrelation; }

  void setRefCorrelation(double refCorrelation) {
    this->refCorrelation = refCorrelation;
  }

  double getRefPartCorrel() const { return refPartCorrel; }

  void setRefPartCorrel(double refPartCorrel) {
    this->refPartCorrel = refPartCorrel;
  }

  int getBin() { return _bin; }

  void setBin(int bin) { _bin = bin; }

  double getExponent() const { return exponent; }

  void setExponent(double exponent) { this->exponent = exponent; }

  ScoreType getScoreType() { return scoreType; }

  void setScoreType(ScoreType newScoreType) { scoreType = newScoreType; }

  TrustLevel getTrust() const { return trust; }

  void setTrust(TrustLevel trust) { this->trust = trust; }

  MatrixPtr getMatrix() { return matrix; }

  static MtzManager *&getReferenceManager() { return referenceManager; }
  ...
  void setRejected(bool rejected) { this->rejected = rejected; }
  ...
  ImagePtr getImagePtr() { return image.lock(); }

  void setImage(ImageWeakPtr imagePtr) { image = imagePtr; }

  void setTimeDelay(double delay) { timeDelay = delay; }

  double getTimeDelay() { return timeDelay; }

  void setParentImageName(std::string pImage) { parentImage = pImage; }

  IndexingSolutionPtr getIndexingSolution() { return indexingSolution; }

  void setIndexingSolution(IndexingSolutionPtr solution) {
    indexingSolution = solution;
  }

  double getSearchSize() { return searchSize; }

  void setRefineOrientations(bool newValue) { refineOrientations = newValue; }
