...
typedef enum {
  CalculationTypeOriginal,
  CalculationTypeIntegrate
} CalculationType;
...
class Miller : public LoggableObject,
               public boost::enable_shared_from_this<Miller> {
 private:
  bool _isSpecial;
  ...
  double superGaussian(double bandwidth, double mean, double sigma,
                       double exponent);
 ...
 public:
  ...
  vec getHKL() { return new_vector(h, k, l); }
  ...
  static double scaleForScaleAndBFactor(double scaleFactor, double bFactor,
                                        double resol,
                                        double exponent_exponent = 1);
  void limitingEwaldWavelengths(vec hkl, double mosaicity, double spotSize,
                                double wavelength, double *limitLow,
                                double *limitHigh, vec *inwards = NULL,
                                vec *outwards = NULL);
  ...
  void setFree(bool newFree) { free = newFree; }
  ...
  double getWeight(bool cutoff = true,
                   WeightType weighting = WeightTypePartialitySigma);
  double resolution();
  ...
  bool isOverlapped();
  void positionOnDetector(double *x = NULL, double *y = NULL,
                          bool search = true);
  ...
  void recalculatePartiality(MatrixPtr rotatedMatrix, double mosaicity,
                             ...
                             bool binary = false, bool no_norm = false);
  double calculatePartiality(double pB, double qB, double beamMean,
  ...
  vec getRay();
  void makeComplexShoebox(double wavelength, double bandwidth, double mosaicity,
                          double rlpSize);
  ...
  ShoeboxPtr getShoebox() { return shoebox; }
  ...
  void setBeam(BeamPtr newBeam) { beam = newBeam; }

  MtzManager *&getMtzParent() { return mtzParent; }

  void setMtzParent(MtzManager *mtz) { mtzParent = mtz; }

  void setPartiality(double partiality) { this->partiality = partiality; }

  double getRawestIntensity();

  double getRawIntensity() const { return rawIntensity * scale; }
  ...
  double getCountingSigma() const { return countingSigma * scale; }

  double getRawCountingSigma() const { return countingSigma; }

  void setCountingSigma(double countingSigma) {
    this->countingSigma = countingSigma;
  }

  bool isNormalised() const { return normalised; }

  void setNormalised(bool normalised) { this->normalised = normalised; }

  MatrixPtr getMatrix() { return matrix; }

  void setMatrix(MatrixPtr matrix) { this->matrix = matrix; }

  void setPolarisationCorrection(double polarisationCorrection) {
    this->polarisationCorrection = polarisationCorrection;
  }

  void setRejected(int rejected) { rejectedReasons = rejected; }

  double getCorrectedX() const { return correctedX; }

  double getCorrectedY() const { return correctedY; }

  void setCorrectedX(double lastX) { this->correctedX = lastX; }

  void setCorrectedY(double lastY) { this->correctedY = lastY; }

  double getPartialCutoff() const { return partialCutoff; }

  void setPartialCutoff(double partialCutoff) {
    this->partialCutoff = partialCutoff;
  }

  double getBFactor() const { return bFactor; }

  void setBFactor(double factor) {
    if (factor == factor) bFactor = factor;
  ...
  void setScale(double scale) {
    if (scale == scale) this->scale = scale;
  }

  double getResolution() { return resolution(); }

  void setResolution(double resol) { this->resol = resol; }

  std::pair<float, float> &getShift() { return shift; }

  float *getXShiftPointer() { return &(shift.first); }

  float *getYShiftPointer() { return &(shift.second); }

  float *getRecipXShiftPtr() { return &recipShiftX; }

  float *getRecipYShiftPtr() { return &recipShiftY; }

  void setShift(const std::pair<int, int> &shift) { this->shift = shift; }

  void setImage(ImagePtr newImage) {
    if (newImage) this->image = newImage;
  }

  ImagePtr getImage() { return image.lock(); }

  void setCorrectingPolarisation(bool on) { correctingPolarisation = on; }

  void setPhase(double newPhase) { phase = newPhase; }

  double getPhase() { return phase; }

  void setWavelength(double wave) { wavelength = wave; }

  int getRejectionFlags() { return rejectedReasons; }

  ReflectionPtr getParentReflection() { return parentReflection.lock(); }

  double getRawSigma() { return sigma; }

  double getPredictedWavelength();

  vec getShiftedRay();

  static void rotateMatrixHKL(double hRot, double kRot, double lRot,
