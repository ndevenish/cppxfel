...
void Hdf5ManagerCheetah::initialiseCheetahManagers() {
  if (cheetahManagers.size() > 0) return;

  std::vector<std::string> hdf5FileGlobs =
  ...
  for (int i = 0; i < hdf5FileGlobs.size(); i++) {
    ...
    logged << "HDF5_SOURCE_FILES entry (no. " << i
           << ") matches: " << std::endl;
  ...
  if (cheetahManagers.size()) {
    logged << "... now managing " << cheetahManagers.size()
           << " hdf5 image source files." << std::endl;
  }
  if (!cheetahManagers.size()) {
    if (hdf5FileGlobs.size()) {
      logged << "You specified some hdf5 image source files but none could be "
                "found. Please check existence and location of files."
             << std::endl;
...
Hdf5ManagerCheetahPtr Hdf5ManagerCheetah::hdf5ManagerForImage(
