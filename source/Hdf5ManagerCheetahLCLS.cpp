...
Hdf5ManagerCheetahPtr Hdf5ManagerCheetahLCLS::makeManager(
    std::string filename) {
  Hdf5ManagerCheetahLCLSPtr cheetahPtr =
      Hdf5ManagerCheetahLCLSPtr(new Hdf5ManagerCheetahLCLS(filename));
...
bool Hdf5ManagerCheetahLCLS::dataForImage(std::string address, void **buffer,
  ...
  std::vector<std::string>::iterator it =
      std::find(imagePaths.begin(), imagePaths.end(), address);
...
void Hdf5ManagerCheetahLCLS::prepareWavelengths() {
  double *tempWaves;
  size_t bytes = Hdf5Manager::hdf5MallocBytesForDataset(wavelengthAddress,
                                                        (void **)&tempWaves);
...
double Hdf5ManagerCheetahLCLS::wavelengthForImage(std::string address,
                                                  void **buffer) {
  std::vector<std::string>::iterator it =
      std::find(imagePaths.begin(), imagePaths.end(), address);
...
int Hdf5ManagerCheetahLCLS::hdf5MallocBytesForImage(std::string address,
                                                    void **buffer) {
  return Hdf5Manager::hdf5MallocBytesForDataset(dataAddress, buffer);
}

size_t Hdf5ManagerCheetahLCLS::bytesPerTypeForImageAddress(
    std::string address) {
