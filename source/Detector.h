...
class Detector : public LoggableObject,
 ...
 public:
  ...
  void initialise(Coord unarrangedTopLeft, Coord unarrangedBottomRight,
                  ...
                  bool lastIsMiddle = false, bool ghost = false);
  ...
  void nudgeTiltAndStep(double *nudgeTiltX, double *nudgeTiltY,
                        double *nudgeStep, double *interNudge = NULL);

  static double lookupCache(double distSq);

  void setCycleNum(int num) { _cycleNum = num; }

  int getCycleNum() { return _cycleNum; }
  ...
  void setArrangedMidPoint(vec midpoint) { arrangedMidPoint = midpoint; }

  vec getArrangedMidPoint() { return arrangedMidPoint; }
  ...
  MatrixPtr getChangeOfBasis() { return changeOfBasisMat; }

  bool didChange() { return _changed; }

  void rotateAxisRecursive(bool fix = false);

  void setParent(DetectorPtr myParent) { parent = myParent; }

  DetectorPtr getParent() { return parent.lock(); }

  bool isLUCA() { return (!parent.lock()); }

  size_t childrenCount() { return children.size(); }

  bool hasNoChildren() { return (childrenCount() == 0); }

  bool hasChildren() { return (childrenCount() > 0); }

  bool isAncestorOf(DetectorPtr detector);

  DetectorPtr getChild(int i) { return children[i]; }
  ...
  static DetectorPtr getMaster() { return masterPanel; }
  ...
  DetectorPtr findDetectorAndSpotCoordToAbsoluteVec(double unarrangedX,
                                                    double unarrangedY,
                                                    vec *arrangedPos);
  ...
  void intersectionWithRay(vec ray, vec *intersection,
                           ImagePtr image = ImagePtr());
  DetectorPtr spotCoordForMiller(MillerPtr miller, double *xSpot,
                                 double *ySpot);
  ...
  size_t millerCount() { return millers.size(); }

  MillerPtr miller(int i) { return millers[i].lock(); }
  ...
  std::string writeGeometryFile(int fileCount, int indentCount = 0,
                                CSVPtr differenceCSV = CSVPtr());
  ...
  static void setNoisy(bool _noisy) { noisy = _noisy; }

  static bool isNoisy() { return noisy; }
  ...
  static ImagePtr getSpecialImage() { return drawImage; }
  ...
  double spotCoordToResolution(double unarrangedX, double unarrangedY,
                               double wavelength);
  void resolutionOrZLimits(double *min, double *max, double wavelength,
                           int type, CSVPtr csv = CSVPtr());
  void zLimits(double *min, double *max);
  void resolutionLimits(double *min, double *max, double wavelength);

  static void setDetectorType(DetectorType _type) { detectorType = _type; }

  static bool isMPCCD() { return (detectorType == DetectorTypeMPCCD); }

  static bool isCSPAD() { return (detectorType == DetectorTypeCSPAD); }

  vec midPointOffsetFromParent(bool useParent = true, bool resetNudge = false);
  void getAllSubDetectors(std::vector<DetectorPtr> &array,
  ...
  bool isSetRefinable() { return _refinable; }
  ...
  void setRefinable(bool refinable) {
    ...
    logged << "Setting " << getTag() << " panel to "
           << (refinable ? "refinable" : "fixed") << std::endl;
    sendLog();
  }
};
