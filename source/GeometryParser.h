...
class GeometryParser : public LoggableObject {
 private:
  ...
  DetectorPtr makeDetectorFromPanelMap(std::vector<PanelMap> panelMaps,
                                       std::string tag, DetectorPtr parent);
  ...
  DetectorPtr makeDetector(DetectorPtr parent, int min_fs, int min_ss,
                           int max_fs, int max_ss, double ss_x, double ss_y,
                           double ss_z, double fs_x, double fs_y, double fs_z,
                           double midpoint_x, double midpoint_y,
                           double midpoint_z, double alpha, double beta,
                           double gamma, bool ghost, bool refinable,
                           double gain);

 public:
  GeometryParser(std::string aFilename,
                 GeometryFormat aFormat = GeometryFormatCrystFEL);
