...
bool SpotVector::isCloseToSpotVector(SpotVectorPtr spotVector2,
                                     double maxDistance) {
  vec spotDiff2 = spotVector2->getVector();

  if (fabs(spotDiff2.h - spotDiff.h) > maxDistance ||
      fabs(spotDiff2.k - spotDiff.k) > maxDistance ||
...
bool SpotVector::hasCommonSpotWithVector(SpotVectorPtr spotVector2) {
  if (firstSpot == spotVector2->firstSpot ||
      firstSpot == spotVector2->secondSpot) {
    return true;
  }

  if (secondSpot == spotVector2->firstSpot ||
      secondSpot == spotVector2->secondSpot) {
    return true;
  }
...
break_out:
  ...
  SpotVectorPtr newVec =
      SpotVectorPtr(new SpotVector(allSpots[0], allSpots[1]));

  return newVec;
}

std::string SpotVector::description() {
  return "(" + f_to_str(spotDiff.h) + ", " + f_to_str(spotDiff.k) + ", " +
         f_to_str(spotDiff.l) + ")";
}

void SpotVector::addSimilarLengthStandardVectors(
    std::vector<SpotVectorPtr> standardVectors, double tolerance) {
  ...
  logged << "Added " << sameLengthStandardVectors.size()
         << " similar standard lengths to vector (min tolerance "
         << getMinDistanceTolerance() << ")." << std::endl;
...
SpotVectorPtr SpotVector::differenceFromVector(SpotVectorPtr spotVec) {
  ...
  SpotVectorPtr newVec =
      SpotVectorPtr(new SpotVector(spotDiffDiff, new_vector(0, 0, 0)));

  return newVec;
}

SpotVectorPtr SpotVector::vectorBetweenSpotsFromArray(
    std::vector<SpotVectorPtr> vectors, SpotPtr spot1, SpotPtr spot2) {
  for (int i = 0; i < vectors.size(); i++) {
    ...
    if (firstSpot == spot1 || firstSpot == spot2) {
      ...
      if ((secondSpot == spot1 || secondSpot == spot2) &&
...
double SpotVector::getResolution() {
  if (approxResolution != 0) return approxResolution;
...
double SpotVector::getMinDistanceTolerance() {
  if (minDistanceTolerance != 0) return minDistanceTolerance;
...
bool SpotVector::isIntraPanelVector() {
  ...
  if ((onePanel != twoPanel) &&
      onePanel->getParent() == twoPanel->getParent()) {
...
void SpotVector::drawOnImage(PNGFilePtr file) {
  ...
  file->drawLine(spot1.first, spot1.second, spot2.first, spot2.second, 0.0, 0,
                 0, 100);
}
