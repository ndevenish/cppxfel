...
DetectorPtr GeometryParser::makeDetector(DetectorPtr parent, int min_fs,
                                         int min_ss, int max_fs, int max_ss,
                                         double ss_x, double ss_y, double ss_z,
                                         double fs_x, double fs_y, double fs_z,
                                         double midpoint_x, double midpoint_y,
                                         double midpoint_z, double alpha,
                                         double beta, double gamma, bool ghost,
                                         bool refinable, double gain) {
...
void GeometryParser::parsePanelListLines(std::vector<std::string> lines) {
  logged << "Warning: 'panels list' definition of prior days of cppxfel is "
            "woefully incomplete."
         << std::endl;
  logged << "If you can parse in a CrystFEL or cppxfel (version 2) geometry "
            "file instead, this"
         << std::endl;
  ...
  std::vector<double> beamCentre =
      FileParser::getKey("BEAM_CENTRE", std::vector<double>(2, 0));
  ...
  for (int i = 0; i < lines.size(); i++) {
    ...
    if (components[0] == "PANEL") {
      ...
      DetectorPtr segment = DetectorPtr(new Detector(
          master, arrangedTopLeft, arrangedBottomRight, sw, o1, o2, gain));
...
void GeometryParser::parseCppxfelLines(std::vector<std::string> lines) {
  ...
  for (int i = 0; i < lines.size(); i++) {
    ...
    if (nextLineShouldBeBrace && components[0] != "{") {
      logged << "Warning: expecting { after start of panel "
             << panelStack.at(panelStack.size() - 1) << "." << std::endl;
      sendLog();
    ...
    if (components[0] == "panel" || components[0] == "}") {
      if (panelStack.size()) {
        ...
        if (!(components[0] == "panel" &&
              detectorStack.size() == panelStack.size())) {
          ...
          DetectorPtr segment = makeDetector(
              parent, min_fs, min_ss, max_fs, max_ss, ss_x, ss_y, ss_z, fs_x,
              fs_y, fs_z, midpoint_x, midpoint_y, midpoint_z, alpha, beta,
              gamma, ghost, refinable, gain);
          segment->setTag(tag);
          ...
          logged << "Added detector " << segment->getTag()
                 << " at hierarchy level " << detectorStack.size() << std::endl;
          sendLog();
        }
      }
    ...
    if (components[0] == "min_fs") min_fs = atoi(components[2].c_str());
    if (components[0] == "min_ss") min_ss = atoi(components[2].c_str());

    if (components[0] == "max_fs") max_fs = atoi(components[2].c_str());
    if (components[0] == "max_ss") max_ss = atoi(components[2].c_str());

    if (components[0] == "fs_x") fs_x = atof(components[2].c_str());
    if (components[0] == "fs_y") fs_y = atof(components[2].c_str());
    if (components[0] == "fs_z") fs_z = atof(components[2].c_str());

    if (components[0] == "ss_x") ss_x = atof(components[2].c_str());
    if (components[0] == "ss_y") ss_y = atof(components[2].c_str());
    if (components[0] == "ss_z") ss_z = atof(components[2].c_str());

    if (components[0] == "midpoint_x") midpoint_x = atof(components[2].c_str());
    if (components[0] == "midpoint_y") midpoint_y = atof(components[2].c_str());
    if (components[0] == "midpoint_z") midpoint_z = atof(components[2].c_str());

    if (components[0] == "alpha") alpha = atof(components[2].c_str());
    if (components[0] == "beta") beta = atof(components[2].c_str());
    if (components[0] == "gamma") gamma = atof(components[2].c_str());
    ...
    if (components[0] == "gain") gain = atof(components[2].c_str());
  }
...
DetectorPtr GeometryParser::makeDetectorFromPanelMap(
    std::vector<PanelMap> panelMaps, std::string tag, DetectorPtr parent) {
...
DetectorPtr GeometryParser::makeDetectorFromPanelMap(PanelMap panelMap,
                                                     DetectorPtr parent) {
  int min_fs;
  int min_ss;
  int max_fs;
  ...
  DetectorPtr segment = DetectorPtr(new Detector(parent));
  segment->initialise(topLeft, bottomRight, slowAxis, fastAxis,
                      arrangedTopLeft);
...
void GeometryParser::parseCrystFELLines(std::vector<std::string> lines) {
  ...
  if (FileParser::getKey("DETECTOR_DISTANCE", 0.) <= 0) {
    ...
    logged
        << "You must specify a detector distance for CrystFEL geometry format!"
        << std::endl;
    sendLogAndExit();
  }
  ...
  for (int i = 0; i < lines.size(); i++) {
    ...
    if (forwardSlashes[0] != lastPanel) {
      ...
      if (laserType == FreeElectronLaserTypeLCLS) {
        laserType =
            (forwardSlashes[0].length() == 2 && forwardSlashes[0][0] == 'q')
  ...
  DetectorPtr master =
      DetectorPtr(new Detector(detectorDistance, beamCentre[0], beamCentre[1]));
  Detector::setMaster(master);
  master->setRefinable(refineQuadrantGeometry);

  logged << "Making the assumption that this detector is a "
         << (isSacla ? "MPCCD" : "CSPAD") << " detector." << std::endl;
  sendLog();
  ...
  if (laserType == FreeElectronLaserTypeSACLA) {
    ...
    DetectorPtr q1256 =
        DetectorPtr(new Detector(master, new_vector(0, 0, 0), "q1256"));
    master->addChild(q1256);
    q1256->setRefinable(refineLocalGeometry);

    DetectorPtr q12 =
        DetectorPtr(new Detector(q1256, new_vector(0, 0, 0), "q12"));
    q1256->addChild(q12);
    ...
    DetectorPtr q56 =
        DetectorPtr(new Detector(q1256, new_vector(0, 0, 0), "q56"));
    q1256->addChild(q56);
    ...
    DetectorPtr q3478 =
        DetectorPtr(new Detector(master, new_vector(0, 0, 0), "q3478"));
    master->addChild(q3478);
    q3478->setRefinable(refineLocalGeometry);

    DetectorPtr q34 =
        DetectorPtr(new Detector(q3478, new_vector(0, 0, 0), "q34"));
    q3478->addChild(q34);
    ...
    DetectorPtr q78 =
        DetectorPtr(new Detector(q3478, new_vector(0, 0, 0), "q78"));
    q3478->addChild(q78);
    q34->setRefinable(refineLocalGeometry);
    ...
    for (int i = 0; i < 16; i++) {
      ...
      DetectorPtr agipdModule = DetectorPtr(
          new Detector(master, new_vector(0, 0, 0), overallPanelName));
      ...
      for (int j = 0; j < 8; j++) {
        ...
        DetectorPtr next =
            makeDetectorFromPanelMap(panelMaps, panelName, agipdModule);
    ...
    DetectorPtr q0_1 =
        DetectorPtr(new Detector(master, new_vector(0, 0, 0), "q0_1"));
    q0_1->setRefinable(refineQuadrantGeometry);
    master->addChild(q0_1);

    DetectorPtr q2_3 =
        DetectorPtr(new Detector(master, new_vector(0, 0, 0), "q2_3"));
    ...
    for (int i = 0; i < quarters.size(); i++) {
      ...
      DetectorPtr qXa0_8 =
          DetectorPtr(new Detector(qX, new_vector(0, 0, 0), qXName + "a0_8"));
      qXa0_8->setRefinable(refineLocalGeometry);
      qX->addChild(qXa0_8);

      DetectorPtr qXa0_3 = DetectorPtr(
          new Detector(qXa0_8, new_vector(0, 0, 0), qXName + "a0_3"));
      qXa0_3->setRefinable(refineLocalGeometry);
      qXa0_8->addChild(qXa0_3);

      DetectorPtr qXa0_1 = DetectorPtr(
          new Detector(qXa0_3, new_vector(0, 0, 0), qXName + "a0_1"));  //
      qXa0_1->setRefinable(refineLocalGeometry);
      qXa0_3->addChild(qXa0_1);
      pairs[i].push_back(qXa0_1);

      DetectorPtr qXa2_3 = DetectorPtr(
          new Detector(qXa0_3, new_vector(0, 0, 0), qXName + "a2_3"));  //
      qXa2_3->setRefinable(refineLocalGeometry);
      qXa0_3->addChild(qXa2_3);
      pairs[i].push_back(qXa2_3);

      DetectorPtr qXa4_7 = DetectorPtr(
          new Detector(qXa0_8, new_vector(0, 0, 0), qXName + "a4_7"));
      qXa4_7->setRefinable(refineLocalGeometry);
      qXa0_8->addChild(qXa4_7);

      DetectorPtr qXa4_5 = DetectorPtr(
          new Detector(qXa4_7, new_vector(0, 0, 0), qXName + "a4_5"));  //
      qXa4_5->setRefinable(refineLocalGeometry);
      qXa4_7->addChild(qXa4_5);
      pairs[i].push_back(qXa4_5);

      DetectorPtr qXa6_7 = DetectorPtr(
          new Detector(qXa4_7, new_vector(0, 0, 0), qXName + "a6_7"));  //
      qXa6_7->setRefinable(refineLocalGeometry);
      qXa4_7->addChild(qXa6_7);
      pairs[i].push_back(qXa6_7);

      DetectorPtr qXa8_15 =
          DetectorPtr(new Detector(qX, new_vector(0, 0, 0), qXName + "a8_15"));
      qXa8_15->setRefinable(refineLocalGeometry);
      qX->addChild(qXa8_15);

      DetectorPtr qXa8_11 = DetectorPtr(
          new Detector(qXa8_15, new_vector(0, 0, 0), qXName + "a8_11"));
      qXa8_11->setRefinable(refineLocalGeometry);
      qXa8_15->addChild(qXa8_11);

      DetectorPtr qXa8_9 = DetectorPtr(
          new Detector(qXa8_11, new_vector(0, 0, 0), qXName + "a8_9"));  //
      qXa8_9->setRefinable(refineLocalGeometry);
      qXa8_11->addChild(qXa8_9);
      pairs[i].push_back(qXa8_9);

      DetectorPtr qXa10_11 = DetectorPtr(
          new Detector(qXa8_11, new_vector(0, 0, 0), qXName + "a10_11"));  //
      qXa10_11->setRefinable(refineLocalGeometry);
      qXa8_11->addChild(qXa10_11);
      pairs[i].push_back(qXa10_11);

      DetectorPtr qXa12_15 = DetectorPtr(
          new Detector(qXa8_15, new_vector(0, 0, 0), qXName + "a12_15"));
      qXa12_15->setRefinable(refineLocalGeometry);
      qXa8_15->addChild(qXa12_15);

      DetectorPtr qXa12_13 = DetectorPtr(
          new Detector(qXa12_15, new_vector(0, 0, 0), qXName + "a12_13"));  //
      qXa12_13->setRefinable(refineLocalGeometry);
      qXa12_15->addChild(qXa12_13);
      pairs[i].push_back(qXa12_13);

      DetectorPtr qXa14_15 = DetectorPtr(
          new Detector(qXa12_15, new_vector(0, 0, 0), qXName + "a14_15"));  //
...
void GeometryParser::parse() {
  ...
  refineQuadrantGeometry =
      !FileParser::getKey("TRUST_QUADRANT_GEOMETRY", false);
  ...
  if (format == GeometryFormatCrystFEL) {
    try {
      ...
      logged
          << "I had trouble trying to parse that geometry format and crashed."
          << std::endl;
      logged << "Sorry. Are you sure the GEOMETRY_FORMAT should be CrystFEL?"
             << std::endl;
      sendLogAndExit();
    }
  }
  ...
  if (format == GeometryFormatCppxfel) {
    try {
      ...
      logged
          << "I had trouble trying to parse that geometry format and crashed."
          << std::endl;
      logged << "Sorry. Are you sure the GEOMETRY_FORMAT should be cppxfel?"
             << std::endl;
      sendLogAndExit();
    }
