...
Shoebox::Shoebox(MillerPtr parent) {
  ...
  bandwidthMultiplier = FileParser::getKey("SHOEBOX_BANDWIDTH_MULTIPLIER",
                                           SHOEBOX_BANDWIDTH_MULTIPLIER);
}
...
void Shoebox::centre(int *centreX, int *centreY) {
  ...
  *centreY = isEven() ? slowSide / 2 : (slowSide - 1) / 2;
  *centreX = isEven() ? fastSide / 2 : (fastSide - 1) / 2;
}
...
void Shoebox::simpleShoebox(int foregroundLength, int neitherLength,
                            int backgroundLength, bool shoeboxEven) {
  ...
  for (int i = centre - neitherLength;
       i <= centre + neitherLength + adjustment2; i++) {
    for (int j = centre - neitherLength;
         j <= centre + neitherLength + adjustment2; j++) {
      shoebox[i][j] = 0;
    }
  }

  for (int i = centre - foregroundLength;
       i <= centre + foregroundLength + adjustment2; i++) {
    for (int j = centre - foregroundLength;
         j <= centre + foregroundLength + adjustment2; j++) {
      shoebox[i][j] = 1;
    }
  }
}

void shoeboxCoordinates(double largeDim, double resolution, int i, int j,
                        double *x, double *y) {
...
void Shoebox::putBoxOnPaddedBackground(Box &smallBox, Box &newShoebox) {
  ...
  for (int i = 0; i < newSlow; i++) {
    ...
    for (int j = 0; j < newFast; j++) {
      ...
      if (i < bgPadding || i >= newSlow - bgPadding || j < bgPadding ||
          j >= newFast - bgPadding) {
        newShoebox[i][j] = -1;
      }

      if (i > bgPadding && i < newSlow - bgPadding - 1 && j > bgPadding &&
          j < newFast - bgPadding - 1) {
        int x = i - bgPadding - 1;
        int y = j - bgPadding - 1;
...
void Shoebox::chopBox(Box &smallBox) {
  for (int i = 0; i < smallBox.size(); i++) {
    ...
    for (int j = 0; j < smallBox[i].size(); j++) {
      if (smallBox[i][j] != 0) allZero = false;
    }
...
void Shoebox::compressBigShoebox(double width, double height, double angle,
                                 Box &smallBox) {
  double maxForegroundDimension = (width > height) ? width : height;
  int maxForegroundDimensionInt = maxForegroundDimension + 1;

  if (maxForegroundDimensionInt % 2 == 0) maxForegroundDimensionInt++;
  ...
  for (int i = 0; i < bufferedMax + 1; i++) bigBox[i].resize(bufferedMax + 1);
  ...
  for (int i = 0; i < bufferedSmall; i++) smallBox[i].resize(bufferedSmall);
...
void Shoebox::complexShoebox(double wavelength, double bandwidth,
                             double radius) {
  ...
  vec hkl =
      new_vector(tempMiller->getH(), tempMiller->getK(), tempMiller->getL());
