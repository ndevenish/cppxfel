...
void MtzRefiner::cycleThread(int offset) {
  ...
  std::vector<int> targets =
      FileParser::getKey("TARGET_FUNCTIONS", std::vector<int>());
...
void MtzRefiner::cycle() {
  ...
  std::ostringstream logged;
  logged << "Filename\tScore type\t\tCorrel\tRfactor\tPart correl\tHits"
         << std::endl;
...
void MtzRefiner::refineCycle(bool once) {
  ...
  bool outputIndividualCycles =
      FileParser::getKey("OUTPUT_INDIVIDUAL_CYCLES", false);
  ...
  while (!finished) {
    ...
    if (once) finished = true;

    if (i == maximumCycles - 1 && stop) finished = true;
...
bool MtzRefiner::loadInitialMtz(bool force) {
  ...
  if (reference && !force) return true;

  logged << "Initial MTZ has " << (hasInitialMtz ? "" : "not ")
         << "been provided." << std::endl;
  sendLog();

  if (hasInitialMtz) {
    std::string referenceFile =
        FileParser::getKey("INITIAL_MTZ", std::string(""));
    ...
    if (reference->reflectionCount() == 0) {
      logged
          << "Initial MTZ reference missing or reflection count is 0. Exiting."
          << std::endl;
      sendLogAndExit();
    }
...
int MtzRefiner::imageSkip(size_t totalCount) {
  ...
  if (skip > totalCount) {
    std::ostringstream logged;
    logged << "IMAGE_SKIP specified is beyond the image count of the available "
              "images."
           << std::endl;
...
void MtzRefiner::readSingleImageV2(std::string *filename,
                                   vector<ImagePtr> *newImages,
                                   vector<MtzPtr> *newMtzs, int offset, bool v3,
                                   MtzRefiner *me) {
  ...
  std::vector<std::string> hdf5Sources =
      FileParser::getKey("HDF5_SOURCE_FILES", std::vector<std::string>());
  ...
  for (int i = offset + skip; i < end; i += maxThreads) {
    if (imageList[i].length() == 0) continue;
    ...
    imgName.erase(std::remove(imgName.begin(), imgName.end(), '\r'),
                  imgName.end());
    ...
    if ((readFromHdf5 && newImages != NULL) || v3) {
      Hdf5ManagerCheetahPtr manager =
          Hdf5ManagerCheetah::hdf5ManagerForImage(imgNameOnly);
    ...
    if (readFromHdf5) {
      Hdf5ImagePtr hdf5Image =
          Hdf5ImagePtr(new Hdf5Image(imgName, wavelength, 0));
      newImage = boost::static_pointer_cast<Image>(hdf5Image);
    } else {
      newImage = ImagePtr(new Image(imgName, wavelength, 0));
    }
    ...
    for (int i = 1; i < lines.size(); i++) {
      ...
      if (components[0] == "wavelength") {
        ...
        if (components.size() >= 2) newWavelength = atof(components[1].c_str());

        newImage->setWavelength(newWavelength);

        logged << "Setting wavelength for " << imgName << " to "
               << newWavelength << " Angstroms." << std::endl;
        Logger::log(logged);
      }
        ...
        if (unitCell) {
          if (newImages) {
            vector<double> correction =
                FileParser::getKey("ORIENTATION_CORRECTION", vector<double>());
            ...
            if (correction.size() >= 2) {
              ...
              if (correction.size() > 2) swivelRot = correction[2] * M_PI / 180;
          ...
          //      if (v3)
          {
            ...
            newManager->setFilename((prefix + "img-" + imgNameOnly + "_" +
                                     i_to_str(currentCrystal) + ".mtz")
                                        .c_str());
            newManager->setMatrix(newMatrix);

            if (setSigmaToUnity) newManager->setSigmaToUnity();
      ...
      if (components[0] == "delay" && newMtzs) {
        if (components.size() > 1) delay = atof(components[1].c_str());
      }
    }
    ...
    if (newMtzs && !v3) {
      ...
      if (!newMatrix) {
        logged << "Warning! Matrix for " << imgName << " is missing."
               << std::endl;
...
void MtzRefiner::readDataFromOrientationMatrixList(
    std::string *filename, bool areImages,
  ...
  if (imageList[0].substr(0, 6) == "ersion") {
    ...
    logged << "Autodetecting matrix list version: " << inputVersion
           << std::endl;
  ...
  for (int i = 0; i < maxThreads; i++) {
    if (version > 1.99 && version < 2.99) {
      ...
      boost::thread *thr =
          new boost::thread(readSingleImageV2, filename, chosenImages,
                            chosenMtzs, i, false, this);
      threads.add_thread(thr);
    } else if (version > 2.99 && version < 3.99) {
      boost::thread *thr =
          new boost::thread(readSingleImageV2, filename, &imageSubsets[i],
                            &mtzSubsets[i], i, true, this);
      threads.add_thread(thr);
    }
  }
...
void MtzRefiner::readMatricesAndImages(std::string *filename, bool areImages,
                                       std::vector<ImagePtr> *targetImages) {
  if (targetImages == NULL && images.size() > 0) return;

  std::string hdf5OutputFile =
      FileParser::getKey("HDF5_OUTPUT_FILE", std::string(""));
  std::vector<std::string> hdf5ProcessingFiles =
      FileParser::getKey("HDF5_SOURCE_FILES", std::vector<std::string>());
  ...
  if (!hdf5) {
    if (aFilename.length() == 0) {
      logged << "No orientation matrix list provided (and no HDF5 file "
                "either). Exiting now."
             << std::endl;
      sendLogAndExit();
    }
  ...
  if (aFilename.length() && !FileReader::exists(aFilename)) {
    std::ostringstream logged;
    logged << "File specified in ORIENTATION_MATRIX_LIST " << aFilename
           << " doesn't exist." << std::endl;

    if (hdf5) {
      logged << "You have specified HDF5 image files, so you don't necessarily "
                "need to specify your ORIENTATION_MATRIX_LIST."
             << std::endl;
    } else {
      logged << "You must either specify some HDF5 image files using "
                "HDF5_SOURCE_FILES, or you must specify "
                "ORIENTATION_MATRIX_LIST to load .img files from the file "
                "system instead."
             << std::endl;
    }
  ...
  bool shouldApplyUnrefinedPartiality =
      FileParser::getKey("APPLY_UNREFINED_PARTIALITY", false);
...
void MtzRefiner::readFromHdf5(std::vector<ImagePtr> *newImages) {
  ...
  double wavelength = FileParser::getKey("INTEGRATION_WAVELENGTH", 0.0);
  double detectorDistance = FileParser::getKey("DETECTOR_DISTANCE", 0.0);
  std::vector<double> beamCentre =
      FileParser::getKey("BEAM_CENTRE", std::vector<double>(0, 2));
  vector<double> givenUnitCell =
      FileParser::getKey("UNIT_CELL", vector<double>());

  int count = 0;

  std::string orientationList =
      FileParser::getKey("ORIENTATION_MATRIX_LIST", std::string(""));
  ...
  if (hdf5OutputFile.length()) {
    hdf5ProcessingPtr =
        Hdf5ManagerProcessingPtr(new Hdf5ManagerProcessing(hdf5OutputFile));
  }
  ...
  if (!loadAll) {
    logged << "Starting from image " << startImage << " and ending on image "
           << endImage << std::endl;
  } else {
  ...
  for (int i = 0; i < inputHdf5Count; i++) {
    ...
    for (int j = 0; j < manager->imageAddressCount(); j++) {
      ...
      if (!hasList) {
        Hdf5ImagePtr hdf5Image =
            Hdf5ImagePtr(new Hdf5Image(imgName, wavelength, detectorDistance));
...
void MtzRefiner::readMatricesAndMtzs() { readMatricesAndImages(NULL, false); }
...
void MtzRefiner::merge(int cycle) {
  ...
  int scalingInt =
      FileParser::getKey("SCALING_STRATEGY", (int)SCALING_STRATEGY);
  ...
  for (int i = 0; i < binList.size(); i++) {
    ...
    merger.setCycle(cycleNum);
    merger.setScalingType(scaling);
    merger.setFilename("allBin" + i_to_str(i) + "_" + i_to_str(cycleNum) +
                       ".mtz");
    merger.mergeFull();
    mergedMtz = merger.getMergedMtz();
  }
...
void MtzRefiner::integrateImagesWrapper(MtzRefiner *object,
                                        vector<MtzPtr> *&mtzSubset,
                                        int offset) {
  object->integrateImages(mtzSubset, offset);
}

void MtzRefiner::integrateImages(vector<MtzPtr> *&mtzSubset, int offset) {
  int maxThreads = FileParser::getMaxThreads();
...
void MtzRefiner::maximumImageThread(MtzRefiner *me, ImagePtr maxImage,
                                    int offset) {
...
void MtzRefiner::maximumImage() {
  ...
  for (int i = 0; i < maxThreads; i++) {
    ImagePtr anImage =
        ImagePtr(new Image("maxImage_" + i_to_str(i) + ".img", 0, 0));
  ...
  for (int i = 0; i < maxThreads; i++) {
    boost::thread *thr =
        new boost::thread(maximumImageThread, this, maxSelections[i], i);
    threads.add_thread(thr);
  }
...
void MtzRefiner::loadImageFiles() {
  ...
  std::string filename =
      FileParser::getKey("ORIENTATION_MATRIX_LIST", std::string(""));
  std::string hdf5 = FileParser::getKey("HDF5_OUTPUT_FILE", std::string(""));
...
void MtzRefiner::integrate() {
  ...
  logged << images.size() << " images with " << crystals
         << " crystal orientations." << std::endl;
  sendLog();
  ...
  for (int i = 0; i < maxThreads; i++) {
    boost::thread *thr =
        new boost::thread(integrateImagesWrapper, this, &managerSubsets[i], i);
    threads.add_thread(thr);
  }
...
void MtzRefiner::integrationSummary() {
  ...
  logged << "N: Spot count plausible for " << goodSpots << " images out of "
         << imageCount << " images (" << pctPlausible << "%)." << std::endl;
  logged << "N: Indexed " << mtzCount << " crystals out of " << imageCount
         << " images (" << pctIndexed << "%)." << std::endl;
  sendLog();

  Logger::mainLogger->addString(
      "Written integration summary to integration.csv");
}
...
void MtzRefiner::loadPanels(bool mustFail) {
  ...
  if (useNewDetectorFormat) {
    ...
    std::string detectorList =
        FileParser::getKey("DETECTOR_LIST", std::string(""));

    logged << "Loading new detector format from " << detectorList << "."
           << std::endl;
    sendLog();
...
void MtzRefiner::writeAllNewOrientations() {
  std::string filename = FileParser::getKey(
      "NEW_MATRIX_LIST", std::string("new_orientations.dat"));
  ...
  for (int i = 0; i < images.size(); i++) {
    ...
    allMats << "distance_offset " << Image::getDistanceOffset(&*images[i])
            << std::endl;
...
void MtzRefiner::writeNewOrientations(bool includeRots, bool detailed) {
  ...
  for (int i = 0; i < mtzManagers.size(); i++) {
    ...
    if (imgFilename.substr(0, 3) == "img") prefix = "";
  ...
  if (!images.size()) {
    ...
    for (int i = 0; i < mtzManagers.size(); i++) {
      ...
      integrateMats << "wavelength " << mtzManagers[i]->getWavelength()
                    << std::endl;
    }
  }
...
void MtzRefiner::index() {
  ...
  if (!indexManager) indexManager = new IndexManager(images);
...
void MtzRefiner::plotPixelValueVsFiducial() {
  ...
  std::vector<int> pix =
      FileParser::getKey("SPECIAL_PIXEL", std::vector<int>(50, 2));
  ...
  for (int i = 0; i < images.size(); i++) {
    ...
    if (fiducial < 0) {
      logged << "Fiducial not found for image " << image->getFilename()
             << std::endl;
      sendLog();
      continue;
    }
...
void MtzRefiner::plotIntegrationWindows() {
  ...
  if (hkl.size() < 3) {
    ...
    std::vector<MtzPtr> mtzManagers = getAllMtzs();
    logged << "Drawing integration windows for " << h << " " << k << " " << l
           << " for " << mtzManagers.size() << " mtzs." << std::endl;
    sendLog();
...
void MtzRefiner::plotIntensities() {
  ...
  if (hkl.size() < 3) {
    logged << "To plot a specific Miller index please use MILLER_INDEX; this "
              "will dump a huge number"
           << std::endl;
    sendLog();
...
void MtzRefiner::displayIndexingHands() {
  ...
  for (int i = 0; i < mtzManagers.size(); i++) {
    std::ostringstream &which =
        mtzManagers[i]->getActiveAmbiguity() == 0 ? idxLogged : invLogged;
...
void MtzRefiner::takeTwoPNG() {
  ...
  if (!image) {
    logged << "No images/special image loaded, cannot do TakeTwo vector plot."
           << std::endl;
    sendLog();
    return;
  }
