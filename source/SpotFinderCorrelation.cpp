...
void SpotFinderCorrelation::findSpecificSpots(std::vector<SpotPtr> *spots) {
  ...
  double minResolution =
      1 / FileParser::getKey("MIN_INTEGRATED_RESOLUTION", 0.);
  double maxResolution =
      1 / FileParser::getKey("MAX_INTEGRATED_RESOLUTION", 1.4);

  std::vector<double> acceptableValue =
      FileParser::getKey("IMAGE_ACCEPTABLE_COMBO", std::vector<double>());
  double gradient = 0;
  double offset = 0;

  if (acceptableValue.size()) {
    gradient = (acceptableValue[1] - minCorrelation) /
               (acceptableValue[0] - threshold);
  ...
  for (int y = 0; y < yDim; y++) {
    for (int x = 0; x < xDim; x++) {
      ...
      if (verbose) {
        logged << "Pixel at (" << x << ", " << y << ") intensity " << value
               << ", correlation " << correlation
               << (success ? " - success" : " - abandoned") << std::endl;
