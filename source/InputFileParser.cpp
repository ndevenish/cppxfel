...
InputFileParser::InputFileParser(std::string filename,
                                 std::vector<std::string> someExtras)
    : FileParser(filename, someExtras) {}
...
void InputFileParser::parseFromPython() { parse(true); }

void InputFileParser::integrate() {
  if (refiner == NULL) parse(true);
...
int InputFileParser::processOptions(std::vector<std::string> lines) {
  ...
  for (int i = 0; i < lines.size(); i++) {
    ...
    if (parserMap.count(command) == 0) {
      logged << "Warning: Line \"" << line << "\" not recognised." << std::endl;
      sendLogAndExit();
    }

    if (function) function(&parameters, command, rest);
  }
...
void InputFileParser::parse(bool fromPython) {
  ...
  //   boost::thread thr = boost::thread(Logger::awaitPrintingWrapper,
  //   Logger::mainLogger);
  ...
  if (fromPython) {
    //      log << "If you are running initial orientation matrix refinement
    //      using image files, please load individual experiments from DIALS
    //      indexing results. If you are running post-refinement on individual
    //      MTZ files, please supply the appropriate orientation matrix list
    //      file on the keyword ORIENTATION_MATRIX_LIST. This command will be
    //      ignored for image files. If you want to load image files please do
    //      so from your Python script." << std::endl;

    log << "Imported commands from parameter file. Waiting for commands."
        << std::endl;
  }
  ...
  if (continueFrom != -1) {
    ...
    log << "No commands issued; defaulting to REFINE_PARTIALITY." << std::endl;
    Logger::mainLogger->addStream(&log);
    log.str("");
    refiner->refine();
  }
