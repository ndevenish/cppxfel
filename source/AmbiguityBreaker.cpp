...
void AmbiguityBreaker::plotDiffOneChipThread(AmbiguityBreaker *me, int offset,
                                             PNGFilePtr png) {
  int maxThreads = FileParser::getMaxThreads();
  std::ostringstream logged;
...
void AmbiguityBreaker::plotDifferences(bool oneChip) {
  ...
  PNGFilePtr png =
      PNGFilePtr(new PNGFile("diffs.png", imageWidth, imageHeight));
...
AmbiguityBreaker::AmbiguityBreaker(vector<MtzPtr> newMtzs) { setMtzs(newMtzs); }
...
void AmbiguityBreaker::setMtzs(vector<MtzPtr> newMtzs) {
  ...
  if (trustAnyway) {
    logged << "TRUST_INDEXING_SOLUTION is set to ON so ambiguity breaking is "
              "skipped."
           << std::endl;
    sendLog();
...
void AmbiguityBreaker::printResults() {
  ...
  for (int i = 0; i < ambiguityCount; i++) {
    logged << "Ambiguity " << i << " : " << ambiguityNums[i] << " crystals."
           << std::endl;
    sendLog();
  }
}
...
void AmbiguityBreaker::breakAmbiguity() {
  ...
  while (!unchanged) {
    ...
    logged << "Cycle " << cycles << " - switched " << changedNum
           << " indexing ambiguities. Best average now " << totalMaxAverage
           << std::endl;
    sendLog();
  }
