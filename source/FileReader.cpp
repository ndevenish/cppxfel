...
vector<std::string> FileReader::split(const std::string s,
...
int FileReader::splitAtIndices(const std::string &s, vector<int> &positions,
                               vector<std::string> &elems) {
  for (int i = 0; i < positions.size() - 1; i++) {
    int start = positions[i];
    int length = positions[i + 1] - start;

    if (s.size() < positions[i + 1]) return 0;
...
std::string FileReader::get_file_contents(const char *filename) {
  ...
  std::string errString =
      "Could not get file contents for file " + std::string(filename);
...
std::string FileReader::addOutputDirectory(std::string filename) {
  std::string directory =
      FileParser::getKey("OUTPUT_DIRECTORY", std::string(""));
