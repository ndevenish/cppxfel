...
void GraphDrawer::resolutionStatsCSV(std::vector<MtzManager *> &managers) {
  ...
  for (int i = 0; i < managers.size(); i++) {
    ...
    if (MtzManager::getReferenceManager()->ambiguityCount() == 2) {
      ...
      if (invCorrel < correl) myMtz->setActiveAmbiguity(0);
    }
    ...
    for (int j = 0; j < bins.size() - 1; j++) {
      ...
      for (int k = 0; k < imgRefls.size(); k++) {
        ...
        if (!imgRefl->anyAccepted()) continue;
...
void GraphDrawer::partialityPNGResolutionShell(
    std::string filename, double meanWavelength,
    std::vector<ReflectionPtr> refRefls, std::vector<ReflectionPtr> imageRefls,
    double minRes, double maxRes) {
  CSVPtr csv =
      CSVPtr(new CSV(4, "wavelength", "intensity", "partiality", "percentage"));
  ...
  logged << csv->entryCount() << " reflections between " << minRes << " and "
         << maxRes << "(mean wavelength " << meanWavelength << ")" << std::endl;
  Logger::log(logged);

  std::string extendedFilename = filename + "_" + f_to_str(minRes, 3) + "_to_" +
                                 f_to_str(maxRes, 3) + "_partiality";
...
void GraphDrawer::partialityPNG(MtzPtr mtz, double maxRes) {
  ...
  std::cout << "Ambiguity 0: " << correl << ", ambiguity 1: " << invCorrel
            << std::endl;

  if (correl > invCorrel) mtz->setActiveAmbiguity(0);
  ...
  for (int i = 0; i < binCount; i++) {
    partialityPNGResolutionShell(mtz->getFilename(), wavelength, refRefls,
                                 imageRefls, resolutions[i],
                                 resolutions[i + 1]);
  }
}
...
void GraphDrawer::plotOrientationStats(vector<MtzPtr> mtzs) {
  ...
  for (int i = 0; i < mtzs.size(); i++) {
    ...
    for (int j = 0; j < lattice->symOperatorCount(); j++) {
      ...
      mat2->preMultiply(*op);
      std::cout << mtzs[i]->getFilename() << "\t" << mat2->summary()
                << std::endl;
...
void GraphDrawer::plotSingleMillerFromMtzs(std::vector<MtzPtr> mtzs, int h,
                                           int k, int l) {
  int index = Reflection::reflectionIdForCoordinates(h, k, l);

  std::ofstream csv;
  csv.open("miller_" + i_to_str(h) + "_" + i_to_str(k) + "_" + i_to_str(l) +
           ".csv");
  csv << "filename,delay,intensity,countingSigma,scale,partiality,phase,"
         "resolution"
      << std::endl;
  ...
  for (int i = 0; i < mtzs.size(); i++) {
    ...
    for (int j = 0; j < mtz->reflectionCount(); j++) {
      ...
      for (int k = 0; k < refl->millerCount(); k++) {
        ...
        csv << mtz->getFilename() << "\t" << mtz->getTimeDelay() << "\t"
            << miller->intensity() << "\t" << miller->getCountingSigma() << "\t"
            << miller->getScale() << "\t" << miller->getPartiality() << "\t"
            << miller->getPhase() << "\t" << 1 / refl->getResolution()
            << std::endl;
      }
    }
  }
...
void GraphDrawer::plotReflectionFromMtzs(std::vector<MtzPtr> mtzs, int h, int k,
                                         int l) {
  if (!(h == 0 && k == 0 && l == 0)) {
...
void GraphDrawer::cutoutIntegrationAreas(std::vector<MtzPtr> mtzs, int h, int k,
                                         int l) {
  int index = Reflection::reflectionIdForCoordinates(h, k, l);
  ...
  for (int i = 0; i < mtzs.size(); i++) {
    ...
    for (int j = 0; j < mtz->reflectionCount(); j++) {
      ...
      for (int k = 0; k < refl->millerCount(); k++) {
        ...
        DetectorPtr detector = Detector::getMaster()->spotCoordForMiller(
            miller, &shiftedX, &shiftedY);

        if (!detector) {
          continue;
        }

        logged << "Found Miller on " << mtz->getFilename()
               << " - will be plotting at (" << shiftedX << ", " << shiftedY
               << ")" << std::endl;
        Logger::log(logged);
        ...
        for (int s = -windowPadding; s < windowPadding + 1; s++) {
          for (int t = -windowPadding; t < windowPadding + 1; t++) {
            ...
            unsigned char pixelValue =
                std::min(value, threshold) * 255 / threshold;
