...
int PNGFile::writeImage(std::string filename, int width, int height,
                        std::string title) {
  int code = 0;
  FILE *fp = NULL;
...
void PNGFile::RGB_to_HSB(float red, float green, float blue, float *brightness,
...
void PNGFile::invertColourRelative(int x, int y) {
  ...
  //      std::cout << (int)red << " " << (int)blue << " " << (int)green <<
  //      std::endl;

  setPixelColour(x, y, 0, 0, 0);
}

void PNGFile::setPixelColourRelative(int x, int y, png_byte red, png_byte green,
                                     png_byte blue) {
  moveCoordRelative(&x, &y);
...
void PNGFile::setPixelColour(int x, int y, png_byte red, png_byte green,
                             png_byte blue, float transparency) {
  int offset = y * bytesPerPixel * pixelsPerRow + x * bytesPerPixel;
...
void PNGFile::drawCircleAroundPixel(int x, int y, float radius,
                                    float transparency, png_byte red,
                                    png_byte green, png_byte blue,
                                    float thickness) {
  moveCoordRelative(&x, &y);
...
void PNGFile::drawText(std::string text, int xCentre, int yCentre, png_byte red,
                       png_byte green, png_byte blue) {
  png_byte *textPixels;
  int width = 0;
  int height = 0;
...
void PNGFile::drawArrow(float xDir, float yDir, float centreX, float centreY,
                        float transparency, png_byte red, png_byte green,
                        png_byte blue) {
  vec points[7];
  ...
  for (float i = -boxWidth; i < +boxWidth; i++) {
    for (float j = -boxWidth; j < +boxWidth; j++) {
      ...
      for (int first = 0; first < 7; first++) {
        ...
        if ((xTest < points[second].h && xTest > points[first].h) ||
            (xTest < points[first].h && xTest > points[second].h)) {
          //              std::cout << i << ", " << j << ", " << "passes between
          //              " << prettyDesc(points[first]) << " and " <<
          //              prettyDesc(points[second]) << "." << std::endl;
          intersections++;
        }
      }
...
void PNGFile::drawLine(int x1, int y1, int x2, int y2, float transparency,
                       png_byte red, png_byte green, png_byte blue) {
...
void PNGFile::dropImage() { free(data); }
