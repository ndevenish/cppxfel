...
class Reflection {
 ...
 public:
  ...
  static int indexForReflection(int h, int k, int l,
                                CSym::CCP4SPG *lowspgroup = NULL,
                                bool inverted = false);
  ...
  double meanIntensityWithExclusion(std::string *filename, int start = 0,
                                    int end = 0);
  ...
  void merge(WeightType weighting, double *intensity, double *sigma,
             bool calculateRejections);
  void medianMerge(double *intensity, double *sigma, int *rejected,
                   signed char friedel);
  void liteMerge(double *intensity, double *countingSigma, double *sigma,
                 int *rejected, signed char friedel = -1);
  ...
  static CSym::CCP4SPG *getSpaceGroup() { return ccp4_space_group; }
  ...
  LiteMiller liteMiller(int i) { return liteMillers[i]; }

  int getActiveAmbiguity() { return activeAmbiguity; }

  void setActiveAmbiguity(int newAmbiguity) { activeAmbiguity = newAmbiguity; }

  const vector<MillerPtr> &getMillers() const { return millers; }

  void setMillers(const vector<MillerPtr> &millers) { this->millers = millers; }

  static MatrixPtr getCustomAmbiguity() { return customAmbiguity; }
  ...
  int liteMillerCount() { return (int)liteMillers.size(); }
