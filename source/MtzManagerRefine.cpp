...
void MtzManager::applyUnrefinedPartiality() {
  ...
  refreshPartialities(0, 0, mosaicity, rlpSize, wavelength, bandwidth,
                      exponent);
}
...
void MtzManager::addParameters(RefinementStrategyPtr refiner) {
  ...
  bool optimisingUnitCellA =
      FileParser::getKey("OPTIMISING_UNIT_CELL_A", false);
  bool optimisingUnitCellB =
      FileParser::getKey("OPTIMISING_UNIT_CELL_B", false);
  bool optimisingUnitCellC =
      FileParser::getKey("OPTIMISING_UNIT_CELL_C", false);

  int function = FileParser::getKey("DEFAULT_TARGET_FUNCTION", 0);
  ScoreType scoreFunc = (ScoreType)function;

  if (optimisingOrientation) {
    refiner->addParameter(this, getHRot, setHRot, stepSizeOrientation,
                          toleranceOrientation, "hRot");
    refiner->addParameter(this, getKRot, setKRot, stepSizeOrientation,
                          toleranceOrientation, "kRot");
  }

  if (optimisingRlpSize) {
    refiner->addParameter(this, getSpotSizeStatic, setSpotSizeStatic,
                          stepSizeRlpSize, toleranceRlpSize, "rlpSize");
  }

  if (optimisingMosaicity) {
    refiner->addParameter(this, getMosaicityStatic, setMosaicityStatic,
                          stepSizeMosaicity, toleranceMosaicity, "mosaicity");
  }

  if (optimisingUnitCellA) {
    refiner->addParameter(this, getUnitCellAStatic, setUnitCellAStatic,
                          stepSizeUnitCellA, 0, "unitCellA");
  }

  if (optimisingUnitCellB) {
    refiner->addParameter(this, getUnitCellBStatic, setUnitCellBStatic,
                          stepSizeUnitCellB, 0, "unitCellB");
  }

  if (optimisingUnitCellC) {
    refiner->addParameter(this, getUnitCellCStatic, setUnitCellCStatic,
                          stepSizeUnitCellC, 0, "unitCellC");
  }

  if (scoreFunc == ScoreTypeReward) {
    double scale = getScale();
    //      refiner->addParameter(this, getScaleStatic, setScaleStatic, scale *
    //      0.25, 0.01, "scale");
  }
  ...
  if (optimisingWavelength) {
    refiner->addParameter(this, getWavelengthStatic, setWavelengthStatic,
                          stepSizeWavelength, toleranceWavelength,
                          "wavelength");
  }

  if (optimisingBandwidth) {
    refiner->addParameter(this, getBandwidthStatic, setBandwidthStatic,
                          stepSizeBandwidth, 0, "bandwidth");
  }

  if (optimisingExponent) {
    refiner->addParameter(this, getExponentStatic, setExponentStatic,
                          stepSizeExponent, 0, "exponent");
  }
}
...
double MtzManager::refinePartialitiesOrientation(int ambiguity, bool reset) {
  ...
  RefinementStrategyPtr refinementMap =
      RefinementStrategy::userChosenStrategy();
...
void MtzManager::refreshCurrentPartialities() {
  if (externalScale != -1) this->applyScaleFactor(externalScale, 0, 0, true);

  applyBFactor(bFactor);
  refreshPartialities(this->hRot, this->kRot, this->mosaicity, this->spotSize,
                      this->wavelength, this->bandwidth, this->exponent);
}
...
void MtzManager::replaceBeamWithSpectrum() {
  ...
  if (gaussian) {
    SpectrumBeamPtr spectrum =
        SpectrumBeamPtr(new SpectrumBeam(gaussian, shared_from_this()));
...
void MtzManager::refreshPartialities(double hRot, double kRot, double mosaicity,
  ...
  for (int i = 0; i < reflections.size(); i++) {
    for (int j = 0; j < reflections[i]->millerCount(); j++) {
      ...
      miller->recalculatePartiality(newMatrix, mosaicity, spotSize, wavelength,
                                    bandwidth, exponent);
    }
  }

  //  logged << "Refreshed partialities with wavelength " << wavelength << ",
  //  spot size " << spotSize << " to generate " << accepted() << " accepted
  //  reflections." << std::endl;

  // sendLog(LogLevelDebug);
}

static bool greaterThan(double num1, double num2) { return (num1 > num2); }

double MtzManager::medianWavelength(double lowRes, double highRes) {
  vector<double> wavelengths;
  double refinementIntensityThreshold =
      FileParser::getKey("REFINEMENT_INTENSITY_THRESHOLD", 200.0);

  vector<double> wavelengthRange =
      FileParser::getKey("WAVELENGTH_RANGE", vector<double>());
  ...
  if (wavelengths.size() < 2) return 0;
...
double MtzManager::bestWavelength(double lowRes, double highRes,
  ...
  if (median) return medianWavelength(lowRes, highRes);
  ...
  if (usingReference) refinementIntensityThreshold = 0.3;

  for (int i = 0; i < reflectionCount(); i++) {
    for (int j = 0; j < reflection(i)->millerCount(); j++) {
      if (reflection(i)->getResolution() < low) continue;

      if (reflection(i)->getResolution() > high) continue;

      double wavelength = reflection(i)->miller(0)->getWavelength();

      if (wavelengthRange.size()) {
        if (wavelength < wavelengthRange[0] ||
            wavelength > wavelengthRange[1]) {
          continue;
        }
      }
      ...
      if (reference != NULL) {
        reference->findReflectionWithId(reflection(i)->getReflId(),
                                        &refReflection);

        if (refReflection &&
            refReflection->meanIntensity() < REFERENCE_WEAK_REFLECTION)
          continue;
      }

      if (usingReference) {
        if (!refReflection) continue;
      ...
      if (isigi > refinementIntensityThreshold && isigi == isigi) {
        totalWavelength += wavelength * weight;
        count += weight;
      }
    }
  }
  ...
  logged << "Best wavelength " << newWavelength << " Å from " << count
         << " reflections." << std::endl;
...
double MtzManager::correlation(bool silent, double lowResolution,
  ...
  if (highResolution == -1) highResolution = maxResolutionAll;

  double correlation = StatisticsManager::cc_pearson(
      this, MtzManager::referenceManager, true, NULL, NULL, lowResolution,
      highResolution, false);

  return correlation;
}

double MtzManager::rSplitWithManager(
    MtzManager *otherManager, bool printHits, bool silent, double lowRes,
...
double MtzManager::correlationWithManager(
    MtzManager *otherManager, bool printHits, bool silent, double lowRes,
    double highRes, int bins,
    vector<boost::tuple<double, double, double, int> > *correlations,
    bool shouldLog, bool freeOnly) {
...
double MtzManager::statisticsWithManager(
    MtzManager *otherManager, StatisticsFunction *function, bool printHits,
    bool silent, double lowRes, double highRes, int bins,
    vector<boost::tuple<double, double, double, int> > *correlations,
    bool shouldLog, bool freeOnly) {
  return statisticsWithManager(otherManager, function, NULL, RFactorNone,
                               printHits, silent, lowRes, highRes, bins,
                               correlations, shouldLog, freeOnly);
}

double MtzManager::rFactorWithManager(
    RFactorType rFactor, bool printHits, bool silent, double lowRes,
    double highRes, int bins,
    vector<boost::tuple<double, double, double, int> > *correlations,
    bool shouldLog) {
  RFactorFunction *function = StatisticsManager::r_factor;
  return statisticsWithManager(NULL, NULL, function, rFactor, printHits, silent,
                               lowRes, highRes, bins, correlations, shouldLog);
}

double MtzManager::statisticsWithManager(
    MtzManager *otherManager, StatisticsFunction *function,
    RFactorFunction *rFactorFunction, RFactorType rFactor, bool printHits,
    bool silent, double lowRes, double highRes, int bins,
    vector<boost::tuple<double, double, double, int> > *correlations,
    bool shouldLog, bool freeOnly) {
  ...
  if (highRes == 0 || highRes < 1 / maxResolution) highRes = 1 / maxResolution;
  ...
  if (rFactor == RFactorNone)
    statistic = function(this, otherManager, !printHits, &hits, &multiplicity,
                         lowRes, highRes, false, freeOnly);
  else
  ...
  if (!silent)
    logged << "N: "
           << "lowRes\thighRes\tValue\tHits\tMultiplicity" << std::endl;

  if (bins > 1 || !silent) {
    StatisticsManager::generateResolutionBins(lowRes, highRes, bins, &shells);

    for (int i = 0; i < shells.size() - 1; i++) {
      double statistic = 0;

      if (rFactor == RFactorNone)
        statistic = function(this, otherManager, 1, &hits, &multiplicity,
                             shells[i], shells[i + 1], false, freeOnly);
      else
      ...
      if (correlations != NULL) {
        boost::tuple<double, double, double, int> result =
            boost::make_tuple(shells[i], shells[i + 1], statistic, hits);
  ...
  if (!silent) {
    ...
    if (rFactor == RFactorNone)
      statistic = function(this, otherManager, 1, &hits, &multiplicity, lowRes,
                           highRes, shouldLog, freeOnly);
    else
      statistic = rFactorFunction(rFactor, this, &hits, &multiplicity, lowRes,
                                  highRes, freeOnly);
...
double MtzManager::rewardAgreement(double low, double high) {
  ...
  this->findCommonReflections(referenceManager, imageRefs, referenceRefs, NULL,
                              true, true);
  ...
  for (int i = 0; i < referenceRefs.size(); i++) {
    ...
    if (imageRef->acceptedCount() == 0) continue;

    if (referenceRef->millerCount() == 0) continue;

    if (imageRef->miller(0)->isFree()) continue;

    if (!referenceRef->betweenResolutions(low, high)) continue;
    ...
    for (int j = 0; j < imageRef->millerCount(); j++) {
      ...
      refInt =
          referenceRef->meanIntensity() * imageRef->miller(j)->getPartiality();
      refSigma = referenceRef->meanSigma();
      weight = referenceRef->meanIntensity() * imageRef->meanPartiality();

      if (myInt == 0 || refInt == 0 || weight != weight ||
          refSigma != refSigma) {
        continue;
      }

      if (myInt != myInt || refInt != refInt) continue;
...
double MtzManager::rSplit(double low, double high) {
  ...
  for (int i = 0; i < referenceRefs.size(); i++) {
    ...
    if (!reverse && imageRef->acceptedCount() == 0) continue;
    ...
    for (int j = 0; j < imageRef->millerCount(); j++) {
      ...
      if (!reverse) {
        ...
        int2 = referenceRef->meanIntensity() *
               imageRef->miller(j)->getPartiality();
        weight = imageRef->meanPartiality();
      }
      ...
      if (int1 != int1 || int2 != int2) continue;

      if (int1 + int2 < 0) continue;
...
double MtzManager::leastSquaresPartiality(double low, double high) {
  ...
  for (int i = 0; i < reflections.size(); i++) {
    ...
    if (refReflection) {
      if (refReflection->meanIntensity() < REFERENCE_WEAK_REFLECTION) continue;

      if (!refReflection->betweenResolutions(low, high)) continue;
  ...
  double minPartiality =
      FileParser::getKey("PARTIALITY_CUTOFF", PARTIAL_CUTOFF);
  ...
  for (int i = 0; i < partials.size(); i++) {
    ...
    if (partials[i].percentage != partials[i].percentage) continue;
  ...
  correl =
      correlation_through_origin(&partialities, &percentages, &intensities);
...
void MtzManager::excludeFromLogCorrelation() {
  ...
  for (int i = 0; i < image1.reflectionCount(); i++) {
    ...
    if (!std::isfinite(int1) || !std::isfinite(int2)) continue;
  ...
  if (imageIntensities.size() < 100) return;

  std::map<double, int> correlationResults;

  double baseCorrelation =
      correlation_between_vectors(&refIntensities, &imageIntensities, &weights);

  if (baseCorrelation > 0.99) return;

  double correlationToGain = 1 - baseCorrelation;

  for (int i = 0; i < refIntensities.size(); i++) {
    double newCorrelation = correlation_between_vectors(
        &refIntensities, &imageIntensities, &weights, i);
    double newCorrelationToGain = 1 - newCorrelation;

    double ratio =
        (correlationToGain - newCorrelationToGain) / correlationToGain;
...
void MtzManager::resetDefaultParameters() {
  ...
  if (alwaysTrust) trust = TrustLevelGood;
}
