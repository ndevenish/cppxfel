...
class Shoebox : LoggableObject {
 private:
  ...
  void compressBigShoebox(double width, double height, double angle,
                          Box &smallBox);
 ...
 public:
  ...
  void printShoebox();
  void simpleShoebox(int foregroundLength, int neitherLength,
  ...
  vector<double> &operator[](std::size_t index) { return shoebox[index]; }

  vector<double> &row(int index) { return shoebox[index]; }

  int height() { return (int)shoebox.size(); }

  MillerPtr getMiller() { return miller.lock(); }

  void setMiller(MillerPtr miller) { this->miller = miller; }

  bool isEven() { return even; }
};
