...
class Image : protected LoggableObject,
              public hasFilename,
 ...
 private:
  ...
  IndexingSolutionStatus extendIndexingSolution(
      IndexingSolutionPtr solutionPtr,
      std::vector<SpotVectorPtr> existingVectors, int *failures = NULL,
      int added = 0);
  ...
  double integrateFitBackgroundPlane(int x, int y, ShoeboxPtr shoebox,
                                     float *error);
  double integrateSimpleSummation(double x, double y, ShoeboxPtr shoebox,
                                  float *error);
  double integrateWithShoebox(double x, double y, ShoeboxPtr shoebox,
                              float *error);
  double weightAtShoeboxIndex(ShoeboxPtr shoebox, int x, int y);
  IndexingSolutionStatus testSeedSolution(
      IndexingSolutionPtr newSolution,
      std::vector<SpotVectorPtr> &prunedVectors, int *successes);
 ...
 protected:
  ...
  void addHighScore(int score) { highScore += score; }

  void checkAndSetupLookupTable();
  static std::mutex setupMutex;
  double standardDeviationOfPixels();
  std::vector<SpotPtr> spots;
  virtual IndexingSolutionStatus tryIndexingSolution(
      IndexingSolutionPtr solutionPtr, bool modify, int *spotsRemoved,
      MtzPtr *anMtz);
  virtual IndexingSolutionStatus tryIndexingSolution(
      IndexingSolutionPtr solutionPtr);
  virtual bool checkIndexingSolutionDuplicates(MatrixPtr newSolution,
 ...
 public:
  ...
  Image(std::string filename = "", double wavelength = 0, double distance = 0);
  void focusOnSpot(int *x, int *y, int tolerance1, int tolerance2);
  void focusOnAverageMax(double *x, double *y, int tolerance1,
                         int tolerance2 = 1, bool even = false);
  void dropImage();
  void newImage();
  virtual ~Image();
  void setUpCrystal(MatrixPtr matrix);
  void addMask(int startX, int startY, int endX, int endY);
  static void applyMaskToImages(vector<ImagePtr> images, int startX, int startY,
                                int endX, int endY);
  std::vector<double> anglesBetweenVectorDistances(double distance1,
                                                   double distance2,
                                                   double tolerance);
  void findSpots();
  ...
  void fakeSpots();
  void integrateSpots();
  void drawMillersOnPNG(PNGFilePtr file, MtzPtr myMtz, char red = 0,
                        char green = 0, char blue = 0);
  ...
  void plotVectorsOnPNG(std::vector<SpotVectorPtr> vectors,
                        std::string filename = "");
  ...
  static ImagePtr getImageMask() { return _imageMask; }

  int getHighScore() { return highScore; }

  std::string getSpotsFile() { return spotsFile; }

  void setSpotsFile(std::string newFile) { spotsFile = newFile; }
  ...
  double intensityAt(double x, double y, ShoeboxPtr shoebox, float *error,
                     int tolerance = 0);
  ...
  void compileDistancesFromSpots(double maxReciprocalDistance = 0,
  ...
  double getSpotVectorWeight() { return spotVectorWeight; }

  void removeCrystal(int j) { mtzs.erase(mtzs.begin() + j); }

  int spotVectorCount() { return (int)spotVectors.size(); }

  SpotPtr spot(int i) { return spots[i]; }

  int spotCount() { return (int)spots.size(); }

  void removeSpot(int i) { spots.erase(spots.begin() + i); }

  SpotVectorPtr spotVector(int i) { return spotVectors[i]; }

  bool hasSeeded() { return _hasSeeded; }

  void setSeeded(bool seed = true) { _hasSeeded = seed; }
  ...
  void setBeamX(double beamX) { this->beamX = beamX; }

  double getBeamY() const { return beamY; }

  void setBeamY(double beamY) { this->beamY = beamY; }

  double getMmPerPixel() const { return mmPerPixel; }

  void setMmPerPixel(double mmPerPixel) { this->mmPerPixel = mmPerPixel; }
  ...
  double getDetectorGain() { return detectorGain; }

  void setDetectorGain(double newGain) { detectorGain = newGain; }

  IndexingSolutionPtr getGoodOrBadSolution(int i, bool good = true) {
    if (good)
      return goodSolutions[i];
    else
      return badSolutions[i];
  }

  int goodOrBadSolutionCount(bool good = true) {
    if (good)
      return (int)goodSolutions.size();
    else
      return (int)badSolutions.size();
  }

  virtual ImageClass getClass() { return ImageClassCppxfel; }

  void addMtz(MtzPtr newMtz) { mtzs.push_back(newMtz); }

  int mtzCount() { return (int)mtzs.size(); }

  MtzPtr mtz(int i) { return mtzs[i]; }
