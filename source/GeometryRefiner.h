...
class GeometryRefiner : public LoggableObject {
 private:
  ...
  int cycleNum;
  bool firstCycle;
  bool refineDetectorStrategy(DetectorPtr detector, GeometryScoreType type,
                              int strategyType);
  static void refineDetectorStrategyWrapper(GeometryRefiner *me,
                                            GeometryScoreType type,
                                            int strategyType);
  static void refineDetectorWrapper(GeometryRefiner *me, int offset,
                                    GeometryScoreType type, int strategyType);
  ...
  RefinementGridSearchPtr makeGridRefiner(DetectorPtr detector,
                                          GeometryScoreType type);
  RefinementStrategyPtr makeRefiner(DetectorPtr detector,
                                    GeometryScoreType type);
  void gridSearchDetectorDistance(DetectorPtr detector, double start,
                                  double end);
