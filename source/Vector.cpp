...
bool within_vicinity(vec vec1, vec vec2, double maxD) {
  if (fabs(vec1.h - vec2.h) > maxD) return false;

  if (fabs(vec1.k - vec2.k) > maxD) return false;

  if (fabs(vec1.l - vec2.l) > maxD) return false;

  take_vector_away_from_vector(vec1, &vec2);

  if (length_of_vector(vec2) > maxD) return false;
...
MatrixPtr closest_rotmat_analytical(vec vec1, vec vec2, vec axis,
                                    double *resultantAngle, bool addPi) {
  ...
  double pPrime =
      (x * x * C + cc) * p + (x * y * C - z * s) * q + (x * z * C + y * s) * r;
  double qPrime =
      (y * x * C + z * s) * p + (y * y * C + cc) * q + (y * z * C - x * s) * r;
  double rPrime =
      (z * x * C - y * s) * p + (z * y * C + x * s) * q + (z * z * C + cc) * r;

  double pDoublePrime = (x * x * oC + occ) * p + (x * y * oC - z * os) * q +
                        (x * z * oC + y * os) * r;
  double qDoublePrime = (y * x * oC + z * os) * p + (y * y * oC + occ) * q +
                        (y * z * oC - x * os) * r;
  double rDoublePrime = (z * x * oC - y * os) * p + (z * y * oC + x * os) * q +
                        (z * z * oC + occ) * r;
...
double angleBetweenVectors(vec vec1, vec vec2, int isUnit) {
  ...
  if (angle != angle && (cosTheta < 1.0001)) angle = 0;
...
double getEwaldSphereNoMatrix(vec index) {
  if (index.l == 0) return 0;

  double ewald_radius =
      index.h * index.h + index.k * index.k + index.l * index.l;
...
double cartesian_to_angle(double x, double y) {
  ...
  if ((x < 0 && y > 0) || (x < 0 && y < 0)) angle += M_PI;

  if (x > 0 && y < 0) angle += M_PI * 2;
...
double super_gaussian(double x, double mean, double sigma_0, double exponent) {
  ...
  double power =
      0 - pow(fabs((x - mean)), exponent) / (2 * pow(sigma, exponent));
...
double correlation_between_vectors(vector<double> *vec1, vector<double> *vec2,
  ...
  for (int i = 0; i < vec1->size(); i++) {
    ...
    if (weights != NULL) addition *= (*weights)[i];
    sum_x += addition;
  ...
  if (mean_x != mean_x || mean_y != mean_y) return 0;
...
double r_factor_between_vectors(vector<double> *vec1, vector<double> *vec2,
                                vector<double> *weights, double scale) {
...
bool higher(double mean1, double mean2) { return mean1 > mean2; }
...
double standard_deviation(vector<double> *values, vector<double> *weights,
  ...
  for (int i = 0; i < values->size(); i++) {
    ...
    if (value != value || value == FLT_MAX) continue;
...
std::map<double, int> histogram(std::vector<double> values, double step) {
  ...
  if (values.size() == 0) return realHistogram;

  // Logger::mainLogger->addString("Histogram has " +
  // std::to_string((int)tempHistogram.size()) + " categories.");

  int minCategory = INT_MAX;
  int maxCategory = -INT_MAX;

  for (std::map<int, int>::iterator it = tempHistogram.begin();
       it != tempHistogram.end(); it++) {
    int category = it->first;

    if (category < minCategory) minCategory = category;
...
void histogramCSV(std::string filename, std::map<double, int> map1,
  ...
  for (std::map<double, int>::iterator it = map1.begin(); it != map1.end();
       it++) {
...
void printDesc(vec hkl) { Logger::mainLogger->addString(desc(hkl)); }
