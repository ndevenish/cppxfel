...
SpectrumBeam::SpectrumBeam(GaussianBeamPtr gaussianStart, MtzPtr mtzStart) {
  ...
  for (int i = 0; i < mtzStart->reflectionCount(); i++) {
    ...
    if (!single->acceptedCount()) continue;

    if (!single->betweenResolutions(lowRes, highRes)) continue;
    ...
    if (!(wavelength == wavelength && ratio == ratio)) continue;
  ...
  /*
  for (double i = originalWavelength - 3 * originalBandwidth; i <=
  originalWavelength + 3 * originalBandwidth; i += originalBandwidth / 6)
  {
...
double SpectrumBeam::interpolateBetweenIterators(
    std::map<double, double>::iterator itLow,
    std::map<double, double>::iterator itHigh, double wavelength) {
...
bool SpectrumBeam::nonZeroPartialityExpected(double lowWavelength,
...
double SpectrumBeam::integralBetweenEwaldWavelengths(double lowWavelength,
                                                     double highWavelength) {
  bool foundHighWavelength = false;
  double summation = 0;

  if (lowWavelength == highWavelength) return 0;

  std::map<double, double>::iterator it = spectrum.end();
  it--;

  if (highWavelength > it->first) return 0;

  for (std::map<double, double>::iterator it = spectrum.begin();
       it != spectrum.end(); it++) {
    std::map<double, double>::iterator it2 = it;
    it2++;

    if (it2 == spectrum.end()) break;
    ...
    if (bottomWavelength != 0 && topWavelength != 0) {
      ...
      //            std::cout << averageValue << ", " << bottomWavelength << ",
      //            " << topWavelength << std::endl;

      //        std::cout << "Sum addition " << addition << " = " << summation
      //        << std::endl;

      summation += addition;
    }

    if (foundHighWavelength) return summation;
  }
