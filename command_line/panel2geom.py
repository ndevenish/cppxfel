...
def find_new_value_as_line(line, panel_num):
  ...
  if variable == "fs" or variable == "ss":
    ...
    line_beginning += sign_x + '{0:.10f}'.format(
        dx) + "x " + sign_y + '{0:.10f}'.format(dy) + "y"
...
for tmpline in template_lines:
  ...
  if len(backslash_split) > 0 and backslash_split[0] == current_panel:
    ...
    print new_line
  elif (len(backslash_split) > 0 and len(backslash_split[0]) >= 4
        and backslash_split[0][0] == "q" and backslash_split[0][2] == "a"):
