...
def matrixForFilename(filename, output):
  ...
  parser = OptionParser(read_experiments=True, check_format=False)
...
def dumpPanels(images):
  ...
  while not successful:
    ...
    try:
      ...
      centre = (x['BEAM_CENTER_X'] / pixelSize[0],
                x['BEAM_CENTER_Y'] / pixelSize[0])
    except Exception as e:
      print Exception
      break
    ...
    for i in range(0, len(panels), 4):
      ...
      print >> panelTxt, panels[i + 1], panels[i + 0], panels[i
                                                              + 3], panels[i
                                                                           + 2]
